 ...
 static async buildBalancePayments(mongoInterface, poolConfig )
  {
    ...
    const customPayoutCache = {}; // cache getWorkerMinPayout per bare address
    ...
    for(let minerData of minerList) //reward each miner
    {
      ...
      if(typeof minerData.tokensAwarded == 'undefined') minerData.tokensAwarded = 0;
       ...
       // Per-miner effective threshold: check main account for custom setting
       const bareKey = minerAddressWithoutWorkerName.toLowerCase();
       if (!(bareKey in customPayoutCache)) {
         customPayoutCache[bareKey] = await PeerHelper.getWorkerMinPayout(minerAddressWithoutWorkerName, mongoInterface);
       }
       const effectiveThreshold = customPayoutCache[bareKey] || min_balance_for_transfer;

       let shouldPayout = false;

        if (typeof num_tokens_owed != 'undefined' && num_tokens_owed > effectiveThreshold) {
          // Regular payout for balances above effective threshold
          shouldPayout = true;
        } else if (isFullPayoutDay && typeof num_tokens_owed != 'undefined' && BigInt(num_tokens_owed) > minBalanceThreshold && num_tokens_owed <= effectiveThreshold) {
          // Daily sweep: pay all balances above 0.01 ETI once per 24h
          shouldPayout = true;
        }
