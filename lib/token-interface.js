 ...
 static async queueMiningSolution(solution_number,minerEthAddress,challenge_digest,challenge_number, randomx_blob, randomx_seedhash, randomxhash, claimedtarget, extraNonce, mongoInterface, poolConfig)
  {
    ...
    var currentMiningTarget = recentMiningContractData.miningTarget
      ...
      var txData= {
        ...
        console.log('- - - - - - - -- - - --- - - - - >queueMiningSolution txData: ', txData);
...
//converted all time miner balance into tokens awarded and creates 'balance payment' records which will get batch-paid 
 static async buildBalancePayments(mongoInterface, poolConfig )
  { 
    ...
    console.log('lastFullPayoutTime: ', lastFullPayoutTime); 
    const currentTime = Date.now();
    let isFullPayoutDay = false;
        if (!lastFullPayoutTime) {
            console.log('case 1 !lastFullPayoutTime is: ', !lastFullPayoutTime); 
            isFullPayoutDay = true;
        } else {
            console.log('case 2 !lastFullPayoutTime: ', !lastFullPayoutTime);
            console.log('case 2 lastFullPayoutTime: ', lastFullPayoutTime.lastFullPayoutTime);
    ...
    console.log('isFullPayoutDay: ', isFullPayoutDay);  
    ...
    for(let minerData of minerList) //reward each miner
    {
      ...
      if(typeof minerData.tokensAwarded == 'undefined') minerData.tokensAwarded = 0;
       ...
       let shouldPayout = false;
        ...
        console.log('For miner:', minerAddress);
        console.log('typeof num_tokens_owed != undefined :', typeof num_tokens_owed != 'undefined');
        console.log('min_balance_for_transfer :', min_balance_for_transfer);
        console.log('num_tokens_owed :', num_tokens_owed);
        console.log('num_tokens_owed > min_balance_for_transfer :', num_tokens_owed > min_balance_for_transfer);
        console.log('shouldPayout is: ', shouldPayout);
    ...
    if (isFullPayoutDay) {
      console.log('update lastFullPayoutTime to currentTime', currentTime);
