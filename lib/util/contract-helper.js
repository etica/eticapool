...
let tokenContractJSON = FileUtils.readJsonFileSync('/contracts/EticaRelease.json');
let paymentContractJSON = FileUtils.readJsonFileSync('/contracts/UniqueBatchedPayments.json');
let deployedContractInfo = FileUtils.readJsonFileSync('/config/DeployedContractInfo.json');
...
export default class ContractHelper {
   ...
   static getTokenContractAddress(poolConfig)
      {
        ...
        var address= deployedContractInfo.networks[networkName].contracts.EticaRelease.blockchain_address;
  ...
  // -----  Returns BatchedPaymentContractAddress contract address: ------  //
  static getBatchedPaymentContractAddress(poolConfig)
      ...
      let networkName = poolConfig.paymentsConfig.networkName
       ...
       // console.log(' --debug-- batchedpaymentsContractAddress is', address); 
        return web3utils.toChecksumAddress(address)
        //console.error('no pool env set', pool_env)
      } 
