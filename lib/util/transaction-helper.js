...
export default class TransactionHelper  {
  ...
  static async submitMiningSolution(minerAddress,solution_number, challenge_number, randomx_blob, randomx_seedhash, randomxhash, claimedtarget, extraNonce, mongoInterface,poolConfig){
    ...
    console.log('-------------- SUBMITING MINING TRANSACTION a1------------');
   ...
   console.log('-challege_number:', challenge_number);
    ...
    const fail_claimtarget = 5000000; // for tests, set a very low claimtarget so that onchain validation fails


    console.log('- _nonce with 0x:', _nonce);


   try {
    // Call checkMintRandomxInputs to validate the inputs
    const checkResult = await tokenContract.methods.checkMintRandomxInputs(randomx_blob, challenge_number, fail_claimtarget, randomx_seedhash).call();
    console.log('checkResult is:', checkResult);
    // Check if the result is valid
    if (!checkResult.isValid) {
        console.log('Input validation failed:', checkResult.errorMessage);
   ...
   var mintMethod = tokenContract.methods.mintrandomX( solution_number, randomx_blob, challenge_number, randomxhash, fail_claimtarget, randomx_seedhash, extraNonce )
   console.log('-------------- SUBMITING MINING TRANSACTION a2------------');
  try{
   ...
   console.log('-------------- SUBMITING MINING TRANSACTION a2.1------------');

   var addressTo = tokenContract.options.address;

   console.log('-------------- SUBMITING MINING TRANSACTION a2.2------------');
   console.log('-------------- SUBMITING MINING TRANSACTION a2.2.a------------');
    ...
    var txData = web3.eth.abi.encodeFunctionCall({
        ...
        }, [_nonce, randomx_blob, challenge_number, randomxhash, fail_claimtarget, randomx_seedhash, _extraNonce ]);

        console.log('-------------- SUBMITING MINING TRANSACTION a2.3------------');
        console.log('-------------- SUBMITING MINING TRANSACTION a2.3------------ txData: ', txData);
   var max_gas_cost = 999000;
 
    var estimatedGasCost = 499000;

    console.log('-------------- SUBMITING MINING TRANSACTION a3------------');
