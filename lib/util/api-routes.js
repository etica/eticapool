...
export function registerApiV2Routes(app, poolConfig, mongoInterface, dataCache) {
  ...
  // 6b. GET /api/v2/pool/miner/:address/shares/chart — pre-computed linedata
  app.get('/api/v2/pool/miner/:address/shares/chart', async (req, res) => {
    try {
      const address = req.params.address.toLowerCase();
      const addressBlockchain = address.toString().substr(0, 42);
      if (!web3utils.isAddress(addressBlockchain)) {
      ...
      const cacheKey = 'miner_shares_chart:' + addressBlockchain;
      let chart = dataCache.get(cacheKey);
      if (!chart) {
        chart = await ChartDataHelper.getInstance().refreshMinerSharesChart(addressBlockchain, mongoInterface);
        if (chart) {
          dataCache.set(cacheKey, chart, 30000); // 30s memory cache
        }
      }
      res.json(chart || { timestamps: [], series: { difficulty: [] }, maxPoints: 100 });
    } catch (err) {
      console.error('GET /api/v2/pool/miner/:address/shares/chart error:', err);
      res.status(500).json({ error: 'Internal server error' });
    }
  });

  // 6c. GET /api/v2/pool/miner/:address/rewards/chart — pre-computed linedata
  app.get('/api/v2/pool/miner/:address/rewards/chart', async (req, res) => {
    try {
      const address = req.params.address.toLowerCase();
      const addressBlockchain = address.toString().substr(0, 42);
      if (!web3utils.isAddress(addressBlockchain)) {
      ...
      const cacheKey = 'miner_rewards_chart:' + addressBlockchain;
      let chart = dataCache.get(cacheKey);
      if (!chart) {
        chart = await ChartDataHelper.getInstance().refreshMinerRewardsChart(addressBlockchain, mongoInterface);
        if (chart) {
          dataCache.set(cacheKey, chart, 30000); // 30s memory cache
        }
      }
      res.json(chart || { timestamps: [], series: { earned: [], poolPercent: [] }, maxPoints: 20 });
    } catch (err) {
      console.error('GET /api/v2/pool/miner/:address/rewards/chart error:', err);
      res.status(500).json({ error: 'Internal server error' });
    }
  });
