...
export function registerApiV2Routes(app, poolConfig, mongoInterface, dataCache) {
  ...
  app.get('/api/v2/account/settings', AuthHelper.authMiddleware, async (req, res) => {
    try {
      const minerData = await mongoInterface.findOne('minerData', { minerEthAddress: req.minerAddress });
      res.json({
  ...
  app.post('/api/v2/account/settings', AuthHelper.authMiddleware, async (req, res) => {
    try {
      const { customMiningDifficulty } = req.body;

      // Validate: must be null (to clear) or a number >= port 3333 minimum (400015)
      // This is the lowest pool port minimum — custom diff below it would have no effect
      const PORT_3333_MIN = 400015;
      if (customMiningDifficulty !== null && customMiningDifficulty !== undefined) {
      ...
      const value = customMiningDifficulty === null || customMiningDifficulty === undefined ? null : Number(customMiningDifficulty);

      await mongoInterface.upsertOne(
        'minerData',
        { minerEthAddress: req.minerAddress },
        { customMiningDifficulty: value }
      );

      res.json({ success: true, customMiningDifficulty: value });
    } catch (err) {
