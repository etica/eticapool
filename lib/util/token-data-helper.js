...
// Heartbeat: log pool status every ~2 minutes (40 x 3s collect cycle)
...
static async getEpochCount(mongoInterface){
 ...
 return recentMiningContractData.epochCount
  ...
  static async collectTokenParameters(tokenContract, web3,  mongoInterface  )
  {
    ...
    _heartbeatCounter++;
    if (_heartbeatCounter >= HEARTBEAT_INTERVAL) {
      _heartbeatCounter = 0;
      try {
        const allMiners = await PeerHelper.getMinerList(mongoInterface);
        let activeMiners = 0;
        let totalHashrate = 0;
        for (const m of allMiners) {
          const hr = parseInt(m.avgHashrate) || 0;
          if (hr > 0) { activeMiners++; totalHashrate += hr; }
        }
        const time = new Date().toISOString().slice(11, 19);
        console.log(`[${time}] Pool OK | epoch ${epochCount} | block ${ethBlockNumber} | diff ${miningDifficulty} | ${activeMiners} miners | ${formatHashrate(totalHashrate)}`);
      } catch (e) {
        // heartbeat is non-critical, don't crash
      }
    }
