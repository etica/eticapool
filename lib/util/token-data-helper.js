...
static async getEpochCount(mongoInterface){
 ...
 return recentMiningContractData.EpochCount
  ...
  static async collectTokenParameters(tokenContract, web3,  mongoInterface  )
  {
    ...
    console.log('Inside collectTokenParameters()')
    ...
    console.log('just before tokenContract.methods.epochCount()');
    
    var epochCount = await tokenContract.methods.epochCount().call();

    console.log('just after tokenContract.methods.epochCount()');
    console.log('just after tokenContract.methods.epochCount():', epochCount);

    console.log('just before tokenContract.methods.randomxBlob()');
    var randomxBlob = await tokenContract.methods.randomxBlob().call();
    console.log('just after tokenContract.methods.randomxBlob()');
    console.log('just after tokenContract.methods.randomxBlob():', randomxBlob);
    console.log('just before tokenContract.methods.randomxSeedhash()');
    var randomxSeedhash = await tokenContract.methods.randomxSeedhash().call();
    console.log('just after tokenContract.methods.randomxSeedhash()');
    console.log('just after tokenContract.methods.randomxSeedhash():', randomxSeedhash);
    ...
    console.log('STAGE collectTokenParameters a1');

    try{ 
      console.log('STAGE collectTokenParameters a2');
        existingMiningContractData = await TokenDataHelper.getRecentMiningContractData(mongoInterface)
        console.log('STAGE collectTokenParameters a3');
        console.log('STAGE collectTokenParameters a3', existingMiningContractData);

    }catch(error){
      console.log('error STAGE collectTokenParameters existingMiningContractData');
      console.log(error)
    }
    console.log('STAGE collectTokenParameters a4');
      var ethBlockNumber = await new Promise(function (fulfilled,error) {
        console.log('STAGE collectTokenParameters a5');
            web3.eth.getBlockNumber(function(err, result)
          {
            if(err){error(err);return}
            console.log('STAGE collectTokenParameters a6');
       ...
       console.log('STAGE collectTokenParameters a7');
    ...
    console.log('STAGE collectTokenParameters a8');  

    if (existingMiningContractData == null ||   challengeNumber != existingMiningContractData.challengeNumber) {

      console.log('INSIDE UPDATE INSERT NEW MINING CONTRACT');
      ...
      console.log('INSIDE UPDATE INSERT NEW MINING CONTRACT:', newMiningContractData);

      console.log('before updated INSIDE UPDATE INSERT NEW MINING CONTRACT:');
      ...
      console.log('after updated INSIDE UPDATE INSERT NEW MINING CONTRACT:');
      ...
      console.log('after urecorded INSIDE UPDATE INSERT NEW MINING CONTRACT:');
      ...
      console.log('just before GeneralEventEmitterHandler.getInstance()');
      GeneralEventEmitterHandler.getInstance().emit('newChallenge', newMiningContractData);
      console.log('just after GeneralEventEmitterHandler.getInstance()');
