...
// ----------  Get Now time info (not blockchain but local using new Date() )  -----------  //
  ...
  { 
    ...
    for(let [index, oneminingcontract] of miningContracts.entries()){
      ...
      challengedetails.miner_challengediff =  await mongoInterface.findOne('miner_challengediff', {challengeNumber: oneminingcontract.challengeNumber, minerEthAddress: minerAddress.toString().toLowerCase(), status:2});
  ...
  static async getMinerPreShares(minerEthAddress, mongoInterface)
  {
    if(minerEthAddress)
    {
      ...
      var sharesData = await mongoInterface.findAllSortedWithLimit("miner_pendingshares", {minerEthAddress: minerEthAddress}, {time:-1},250 );
  ...
  */
  ...
  static async getTokenRewardForShareOfDifficulty(shareDiff, totalDiff, minerport, poolConfig, mongoInterface){
   ...
   // --> totalDiff is totalDifficulty.totaldiff
    ...
    if(minerport == 8080){
      totalBlockReward = (totalBlockReward * 15) / 100; // low difficulty shares get 15% of rewards
    }
    if(minerport == 8081){
