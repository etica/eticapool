...
// ----------  Get Now time info (not blockchain but local using new Date() )  -----------  //
  ...
  static async getMinerDataWithWorkers(minerEthAddress, mongoInterface)
  {
    ...
    if(minerEthAddress)
    {
      ...
      if(minerData  == null)
      {
        ...
        // Re-fetch the newly created record but continue to worker-fetching logic below
        // (old code returned early here, skipping workers entirely on first call)
        minerData = await mongoInterface.findOne("minerData", {minerEthAddress: minerEthAddress.toString().toLowerCase() } );
  ...
  static async getTotalMinerHashrate(mongoInterface)
  {
    ...
    { 
       ...
       var hashrate = parseInt(minerData.avgHashrate)
  ...
  // -------  Updates alltimeTokenBalance field (with value tokenRewardAmt) for a specific Miner on Mongo DB -------- //
    ...
    static async increaseTokensReceivedForMiner( minerEthAddress, balance_payment, mongoInterface)
    {
     ...
     var minerData = await PeerHelper.getMinerData(minerEthAddress,mongoInterface)
      ...
      console.log('[PAYMENT-TRACE] increaseTokensReceivedForMiner: minerAddress:', minerEthAddress?.substring(0,20), '| minerData._id:', minerData._id, '| currentTokensReceived:', tokensReceived, '| amountToPay:', balance_payment.amountToPay, '| newTotal:', tokensReceived + balance_payment.amountToPay);
      await mongoInterface.updateOneCustom('minerData', {_id: minerData._id},
              {$inc:{tokensReceived: balance_payment.amountToPay}}   )

        console.log('[PAYMENT-TRACE] increaseTokensReceivedForMiner: $inc DONE for', minerEthAddress?.substring(0,20));
  ...
  */
  ...
  static async getPoolFeesMetrics(poolConfig, mongoInterface){
    ...
    if(!token_Eth_Price_Ratio || token_Eth_Price_Ratio == 0){
      console.log('WARN: Missing price oracle for pool fees factor, using defaults')
      token_Eth_Price_Ratio = 1; // default to 1:1 ratio so reward calculation still works
    }
  ...
  static async getMaxGweiPriceUntilMiningSuspension(poolConfig, mongoInterface){
   ...
   // Look up customMinPayout from the main account (bare address) for any worker
   static async getWorkerMinPayout(bareAddress, mongoInterface) {
     const addr = bareAddress.toString().substr(0, 42).toLowerCase();
     const mainAccount = await mongoInterface.findOne('minerData', { minerEthAddress: addr });
     if (mainAccount && mainAccount.customMinPayout && mainAccount.customMinPayout > 0) {
       return mainAccount.customMinPayout;
     }
     return null;
   }
   ...
   // -------  Returns active Miners for frontend display (optimized)  -------- //
   static  async getMinerListForDisplay( mongoInterface, limit, skip )
    {
        ...
        var projection = {
            ...
            minerEthAddress: 1,
            avgHashrate: 1,
            alltimeTokenBalance: 1,
            tokensReceived: 1,
            entryport: 1,
            last24hTokenBalance: 1,
            lastSubmittedSolutionTime: 1
        };

        let minerData = await mongoInterface.findAllSortedWithLimitAndProjection(
            "minerData",
            { avgHashrate: { $gt: 0 } },
            { avgHashrate: -1 },
            limit,
  ...
  // --------  Targeted $set for payment fields only (avoids overwriting concurrent $inc on alltimeTokenBalance)  ---------  //
  static async updateMinerPaymentFields(minerEthAddress, tokensAwarded, mongoInterface)
  {
    if(minerEthAddress == null) return;

    minerEthAddress = minerEthAddress.toString().toLowerCase()

    return await mongoInterface.updateOne("minerData",
        {minerEthAddress: minerEthAddress},
        {tokensAwarded: tokensAwarded, tokenBalance: 0}
    );
  }
  // --------  Targeted $set for payment fields only  ---------  //
...
static async calculatePoolhashrate(mongoInterface, poolConfig) {
  ...
  const poolNumberworkers = await PeerHelper.getPoolnumberworkers(mongoInterface)
  ...
  let newpoolrecord = {};
      ...
      if (poolNumberworkers > 0) {
        newpoolrecord.Numberworkers = poolNumberworkers;
      }
      else {
      newpoolrecord.Numberworkers = 0;
      }
      ...
      // Update pre-computed chart linedata
      try {
        await ChartDataHelper.getInstance().updateChart('pool_hashrate_24h', {
          timestamp: newpoolrecord.recordedat,
          hashrate: newpoolrecord.Hashrate,
          miners: newpoolrecord.Numberminers,
          workers: newpoolrecord.Numberworkers,
        }, mongoInterface);

        // Emit chartUpdate event for Socket.IO broadcast
        const handler = GeneralEventEmitterHandler.getInstance();
        handler.emit('chartUpdate', {
          name: 'pool_hashrate_24h',
          point: {
            timestamp: newpoolrecord.recordedat,
            hashrate: newpoolrecord.Hashrate,
            miners: newpoolrecord.Numberminers,
        ...
        console.error('Chart linedata update error:', err);
      }
...
static async getPoolnumberminers(mongoInterface) {
  // Count distinct addressBlockchain values among all active minerData docs
  // This ensures an address with only named workers (no main doc active) still counts as 1 miner
  let result = await mongoInterface.aggregateOnCollection("minerData", [
    { $match: { avgHashrate: { $gt: 0 } } },
    { $group: { _id: "$addressBlockchain" } },
    { $count: "total" }
  ], {});
...
static async getPoolnumberworkers(mongoInterface) {
  let numberWorkers = await mongoInterface.countAll("minerData", {avgHashrate: {$gt: 0}});
