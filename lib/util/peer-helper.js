  ...
  */
  ...
  static async getMaxGweiPriceUntilMiningSuspension(poolConfig, mongoInterface){
   ...
   // -------  Returns all Shares  -------- //
      ...
      {
          console.log('getting getMintAddresses')
          let mintAddresses = await mongoInterface.findAll( "network_pools_addresses", {} )
          
          console.log('getting getMintAddresses', mintAddresses)
...
// ------------------  Added thanks to crnxhh --------------- //
     ...
     static async calculateMinerHashrateData(mongoInterface, poolConfig) {
         ...
         console.log('active minerList is: ', minerList)
         console.log('PeerHelper.getTimeNowSeconds()', PeerHelper.getTimeNowSeconds())
         console.log('PeerHelper.getTimeNowSeconds() - HashRateCalculationPeriod', PeerHelper.getTimeNowSeconds() - HashRateCalculationPeriod)
         ...
         for (let minerData of minerList) {
             ...
             if (minerTotalDifficulty) {
              console.log('minerTotalDifficulty is: ', minerTotalDifficulty)
                 ...
                 console.log('avgHashrate is: ', avgHashrate)
                 let convertedHashrate = avgHashrate.toNumber(); // MH unit
                 console.log('convertedHashrate is: ', convertedHashrate)
                 avgHashrate = parseInt((convertedHashrate).toFixed(6));
                 console.log('avgHashrate II is: ', avgHashrate)
                 if (avgHashrate !== minerData.avgHashrate) {
                     ...
                     console.log('lastpreshareArray is: ', lastpreshareArray)
                     if(lastpreshareArray == null || lastpreshareArray.length<=0){
                      console.log('lastpreshareArray conds pass was true')
                      // Should never happen but just in case, If no lastpreshareArray it means skip this miner and continue loop
                      continue
                    }
                    console.log('lastpreshareArray conds pass was false')
                    console.log('updating minerData avgHashrate is: ', avgHashrate)
                    console.log('updating minerData minerData._id is: ', minerData._id)
              ...
              console.log('no minerTotalDifficulty minerData: ', minerData)
...
static async calculateNetworkhashrate(mongoInterface, poolConfig) {
  ...
  if(poolHashrate && poolHashrate[0] && !isNaN(poolHashrate[0].Hashrate)){
       console.log('poolHashrate[0].Hashrate is: ', poolHashrate[0].Hashrate);
       networkHashrate = poolHashrate[0].Hashrate;
  }

  if(poolNumberminers && poolNumberminers[0] && !isNaN(poolNumberminers[0].Numberminers)){
       console.log('poolNumberminers[0].Numberminers is: ', poolNumberminers[0].Numberminers);
       networkNumberminers = poolNumberminers[0].Numberminers;
  } 
  ...
  try {
    ...
    if (onepool.Hashrate && !isNaN(onepool.Hashrate)) {
      console.log('onepool.Hashrate is: ', onepool.Hashrate);
      networkHashrate += onepool.Hashrate;
    }

    if (onepool.Numberminers && !isNaN(onepool.Numberminers)) {
      console.log('onepool.Numberminers is: ', onepool.Numberminers);
