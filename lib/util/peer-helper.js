...
static async calculateMinerHashrateData(mongoInterface, poolConfig) {
  ...
  const HashRateCalculationPeriod_extendedMargin = HashRateCalculationPeriod * 2;
  
  try {
      // Get all active miners and miners that need to be turned off
      const [minerList, needturnoffMiners] = await Promise.all([
          PeerHelper.getActiveMinerList(HashRateCalculationPeriod_extendedMargin, mongoInterface),
          PeerHelper.getNeedTurnOffMinerList(HashRateCalculationPeriod_extendedMargin, mongoInterface)
      ]);

      const startTime = PeerHelper.getTimeNowSeconds() - HashRateCalculationPeriod;

      // Get total difficulty per miner for the last hour
      ...
      // Add updates for miners that need to be turned off
      needturnoffMiners.forEach(miner => {
          bulkUpdates.push({
              filter: { _id: miner._id },
              update: { $set: { avgHashrate: 0 } }
          });
      });

      // Process active miners
      for (let minerData of minerList) {
          const minerTotalDifficulty = sharesDifficultySum.find(o => o._id === minerData.minerEthAddress);
          
          if (minerTotalDifficulty) {
              let avgHashrate = web3utils.toBN(minerTotalDifficulty.difficulty)
                  .div(web3utils.toBN(HashRateCalculationPeriod))
                  .toNumber();
              
              avgHashrate = parseInt(avgHashrate.toFixed(6));

              if (avgHashrate !== minerData.avgHashrate) {
                  const lastpreshareArray = await PeerHelper.getLastMinerPreShare(minerData.minerEthAddress, mongoInterface);
                  if (lastpreshareArray && lastpreshareArray.length > 0) {
                      bulkUpdates.push({
                          filter: { _id: minerData._id },
                          update: { 
                              $set: {
                                  avgHashrate: avgHashrate,
                                  minerport: lastpreshareArray[0].minerport,
                                  entryport: lastpreshareArray[0].entryport
                              }
                          }
                      });
                  }
      ...
      if (bulkUpdates.length > 0) {
          await mongoInterface.bulkUpdate('minerData', bulkUpdates);
      }
...
/* New hashrate calculation method, not used 
static async calculateMinerHashrateData(mongoInterface, poolConfig) {
  const HashRateCalculationPeriod = 60 * 60;  // 60 minutes
