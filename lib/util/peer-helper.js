...
static async calculateLast24hEarnings(mongoInterface) {
  try {
    ...
    const LastEarningsSum = await PeerHelper.getLastEarningsSum3(mongoInterface, (PeerHelper.getTimeNowSeconds() - EarningsCalculationPeriod))
...
static async getLastEarningsSum3(mongoInterface, unixTime) {
  ...
  let minerEarningsSum = await mongoInterface.aggregateOnCollection("ppnls_rewards", [
    { $match: { createdAt: { $gt: unixTime } } },
    {
      $group: {
        _id: {
          $substrCP: ["$minerEthAddress", 0, { $indexOfBytes: ["$minerEthAddress", "."] }]
        },
