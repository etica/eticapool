...
// ----------  Get Now time info (not blockchain but local using new Date() )  -----------  //
  ...
  // ----------  Hard: Get Pool Minimum Difficulty Hard and Pool Minimum Share Target from parameter object poolConfig -----------  //
      ...
      { 
         ...
         console.log('poolConfig diff is:', diff);
   ...
   // ----------  Get Current Target from Difficulty -----------  //
    static getTargetFromDifficulty(difficulty)
    {
      ...
      console.log(' getTargetFromDifficulty() - - - - - - - - > max_target:',  max_target.toString());
  ...
  {
    console.log('STAGE XI awardTokensBalanceForShares');
...
// ------------------  Added thanks to crnxhh --------------- //
     ...
     static async calculateMinerHashrateData(mongoInterface, poolConfig) {
      console.log('----------------------------------- *-*-**-*-*-*-*-**- INSIDE calculateMinerHashrateData *-*-**-*-*-*-*-* ---------------------------------');
         ...
         console.log('active miners are: ', minerList);
         console.log('needturnoffMiners: ', needturnoffMiners);
         ...
         for (let minerData of minerList) {
          console.log('in forloop for minerData: ', minerData);
             let minerTotalDifficulty = sharesDifficultySum.find(o => o._id === minerData.minerEthAddress);
             console.log('in forloop minerTotalDifficulty: ', minerTotalDifficulty);
             if (minerTotalDifficulty) {
                 ...
                 console.log('in forloop avgHashrate: ', avgHashrate);
                 if (avgHashrate !== minerData.avgHashrate) {
                  console.log('*-*-*-** 66666666 -*-*-*-*-*- in forloop avgHashrate !== minerData.avgHashrate passed');
                     var lastpreshareArray = await PeerHelper.getLastMinerPreShare(minerData.minerEthAddress, mongoInterface);
                     console.log('in forloop lastpreshareArray: ', lastpreshareArray);
                     if(lastpreshareArray == null || lastpreshareArray.length<=0){
                      console.log('*-*-*-** 88888888888888 -*-*-*-*-*- in forloop lastpreshareArray == null || lastpreshareArray.length<=0 passed');
                      // Should never happen but just in case, If no lastpreshareArray it means skip this miner and continue loop
                      continue
                    }
                    console.log('*-*-*-** 9999999 -*-*-*-*-*- in forloop just before updating miner hashrate with: ', avgHashrate);
         ...
         for (let needturnoffMiner of needturnoffMiners) {
          ...
          console.log('Turning off miner: ', needturnoffMiner);
