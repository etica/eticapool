  ...
  // ----------  Hard: Get Pool Minimum Difficulty Hard and Pool Minimum Share Target from parameter object poolConfig -----------  //
   ...
   // ----------  Get Current Target from Difficulty -----------  //
    static getTargetFromDifficulty(difficulty)
    {
      ...
      console.log(' getTargetFromDifficulty() - - - - - - - - > max_target:', web3utils.toBN( difficulty).toString());
    
      var max_target = web3utils.toBN('2').pow(web3utils.toBN('256')).sub(web3utils.toBN('1'));

      console.log(' getTargetFromDifficulty() - - - - - - - - > max_target:',  max_target);
 
      var current_target = max_target.div( web3utils.toBN( difficulty) );

      console.log(' getTargetFromDifficulty() - - - - - - - - > current_target:',  current_target);
