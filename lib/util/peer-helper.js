...
// ----------  Get Now time info (not blockchain but local using new Date() )  -----------  //
  ...
  {
    console('debug11 --calling getTotalMinerShares() with PeerHelper.getMinerList()')
  ...
  {
    console('debug11 --calling getTotalMinerShares() with PeerHelper.getTotalMinerHashrate')
    var allMinerData  = await PeerHelper.getMinerList(mongoInterface)
   ...
   static  async getMinerList( mongoInterface )
    {
      ...
      console('debug11 --inside PeerHelper.getMinerList()')
        let minerData = await mongoInterface.findAll( "minerData", {} )
        console('debug11 --inside passed PeerHelper.getMinerList()')  
        return minerData;
