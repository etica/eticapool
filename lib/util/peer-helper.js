  ...
  static async getMinerDataWithWorkers(minerEthAddress, mongoInterface)
  {
    ...
    if(minerEthAddress)
    {
      ...
      if(minerData  == null)
      {
        ...
        minerData.totalAvgHashrate = minerData.avgHashrate + totalAvgHashrate;
        minerData.totalTokensAwarded = minerData.tokensAwarded + totalTokensAwarded;
        minerData.totalTokensReceived = minerData.tokensReceived + totalTokensReceived;
        minerData.totalAlltimeTokenBalance =  minerData.alltimeTokenBalance + totalAlltimeTokenBalance;
  ...
  static async awardTokensBalanceForShares( minerEthAddress, difficulty , totaldiff, minerport, poolmint, challengenumber, poolConfig, mongoInterface)
  {
   ...
   //var sharesData = await PeerHelper.getSharesData(minerEthAddress,mongoInterface)
    ...
    await mongoInterface.updateOneCustom('minerData', {_id: minerData._id}, 
            ...
            if(miner.addressBlockchain){
              addressBlockchain = miner.addressBlockchain;
            }
...
static async calculateLast24hEarnings(mongoInterface) {
  try {
    ...
    console.log('calculating Last24hEarnings');
    ...
    const LastEarningsSumMainAddresses = await PeerHelper.getLastEarningsSumMainAddresses(mongoInterface, (PeerHelper.getTimeNowSeconds() - EarningsCalculationPeriod))

    console.log('LastEarningsSum  is: ', LastEarningsSum);
    console.log('LastEarningsSumMainAddresses  is: ', LastEarningsSumMainAddresses);

    // Get the current time and calculate the timestamp for 24 hours ago
    const now = new Date();

    console.log('calculating Last24hEarnings II');

    // Update each miner's Last24hTokenBalance in minerData
    for (let earning of LastEarningsSum) {
      console.log('updating an earning');
      console.log('updating earning:', earning);

      const _addressBlockchain = earning._id.toString().substr(0, 42);
      console.log('_addressBlockchain is:', _addressBlockchain);
      // not a main address (uses worker name), update minerData:
      if(_addressBlockchain != earning._id){
        console.log('_addressBlockchain different earning._id, _addressBlockchain from is:', _addressBlockchain);
        console.log('_addressBlockchain different earning._id, earning._id from is:', earning._id);
        ...
        console.log('_addressBlockchain same as earning._id, _addressBlockchain from is:', _addressBlockchain);
        console.log('_addressBlockchain same as earning._id, earning._id from is:', earning._id);
      }

    }

    for (let earning of LastEarningsSumMainAddresses) {
      console.log('updating an earning of LastEarningsSumMainAddresses');
      console.log('updating earning LastEarningsSumMainAddresses:', earning);
      await mongoInterface.updateOneCustom(
        'minerData',
        { minerEthAddress: earning._id },
        { $set: { Last24hTokenBalance: earning.totalTokensAwarded } }
...
static async getLastEarningsSum(mongoInterface, unixTime) {
  console.log('calling getLastEarningsSum');
  let minerEarningsSum = await mongoInterface.aggregateOnCollection("ppnls_rewards",
      [{$match: {createdAt: {$gt: unixTime}}},
          {
              $group:
...
static async resetLast24hNoEarnings(mongoInterface) {
  try {
    console.log('Resetting Last24hNoEarnings for inactive miners');
    ...
    for (let miner of activeMiners) {
      ...
      console.log('recentRewards -- is:', recentRewards);
      ...
      if (!recentRewards) {
        ...
        console.log(`Reset Last24hTokenBalance for miner: ${miner.minerEthAddress}`);
      }
    }

    console.log("Reset of Last24hEarnings for inactive miners completed successfully.");
  } catch (error) {
