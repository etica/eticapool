...
// ----------  Get Now time info (not blockchain but local using new Date() )  -----------  //
  ...
  {
    console.log('debug11 --calling getTotalMinerShares() with PeerHelper.getMinerList()')
  ...
  {
    console.log('debug11 --calling getTotalMinerShares() with PeerHelper.getTotalMinerHashrate')
    var allMinerData  = await PeerHelper.getMinerList(mongoInterface)
  ...
  {
   console.log('awardTokensBalanceForShares called minerEthAddress: ', minerEthAddress, 'difficulty; ', difficulty, 'totaldifficulty:', totaldiff)
   ...
   //var sharesData = await PeerHelper.getSharesData(minerEthAddress,mongoInterface)
    ...
    await mongoInterface.updateOneCustom('minerData', {_id: minerData._id}, 
            ...
            var PpnlsReward =  {
             ...
             console.log('inserting PpnlsReward: ', PpnlsReward);
  ...
  */
  ...
  static async getTokenRewardForShareOfDifficulty(shareDiff, totalDiff, minerport, poolConfig, mongoInterface){
   ...
   // --> totalDiff is totalDifficulty.totaldiff
    ...
    if(netReward > 31963444048310830000){
      netReward = 31963444048310830000; // 31.963 eti (current block reward) max to protect against any unpredicted bug
    }
  ...
  static async getMaxGweiPriceUntilMiningSuspension(poolConfig, mongoInterface){
   ...
   static  async getMinerList( mongoInterface )
    {
      ...
      console.log('debug11 --inside PeerHelper.getMinerList()')
        let minerData = await mongoInterface.findAll( "minerData", {} )
        console.log('debug11 --inside passed PeerHelper.getMinerList()')  
        return minerData;
   ...
   // -------  Returns all Shares  -------- //
     ...
     {
         let ppnlsRewards = await mongoInterface.findAllSortedWithLimit( "ppnls_rewards", {minerEthAddress: minerAddress.toString().toLowerCase()}, {epochCount:-1}, nbrewards );
         console.log('ppnlsRewards are :::', ppnlsRewards);
         return ppnlsRewards;
     } 
