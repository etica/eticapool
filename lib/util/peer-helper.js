...
// ----------  Get Now time info (not blockchain but local using new Date() )  -----------  //
  ...
  static async awardTokensBalanceForShares( minerEthAddress, difficulty , totaldiff, minerport, poolmint, challengenumber, poolConfig, mongoInterface)
  {
   ...
   //var sharesData = await PeerHelper.getSharesData(minerEthAddress,mongoInterface)
    ...
    let [tokenRewardAmt, bonusRewardAmt, bonusRate] = await PeerHelper.getTokenRewardForShareOfDifficulty(difficulty, totaldiff, minerport, poolConfig, mongoInterface)
    ...
    let  bonusRateAwarded = parseInt (bonusRate); // bonusRate percent, example 20 for 20% bonus, 0% if no bonus
    ...
    await mongoInterface.updateOneCustom('minerData', {_id: minerData._id}, 
            ...
            var PpnlsReward =  {
             ...
             if (bonusAwarded > 0 && !(isNaN(bonusAwarded))) {
              PpnlsReward.bonusRateAwarded = bonusRateAwarded;
  ...
  */
  ...
  static async getTokenRewardForShareOfDifficulty(shareDiff, totalDiff, minerport, poolConfig, mongoInterface){
    ...
    console.log('9-9--9-9-9-9-9-9-,9-,99- --- getTokenRewardForShareOfDifficulty');
   ...
   // --> totalDiff is totalDifficulty.totaldiff
    ...
    console.log('9-9--9-9-9-9-9-9-,9-,99- --- poolRewardsBonus', poolRewardsBonus);
    ...
    if(poolRewardsBonus && poolRewardsBonus > 0){
      ...
      console.log('9-9--9-9-9-9-9-9-,9-,99- --- applying Block Reward Bonus');
      console.log('netReward: ', netReward);
      console.log('poolRewardsBonus: ', poolRewardsBonus);
      console.log('netBonus: ', netBonus);
    }
    else {
      console.log('9-9--9-9-9-9-9-9-,9-,99- ---  NOT applying Block Reward Bonus');
      console.log('netReward: ', netReward);
      console.log('poolRewardsBonus: ', poolRewardsBonus);
      console.log('netBonus: ', netBonus);
    }

    console.log('new netBonus: ', netBonus);
    ...
    return netReward, netBonus, poolRewardsBonus
  }
  // -------  Returns netReward for specific miner based on its sharediff compared to totaldiff. taking into Bloackreward, account Pool fees and shareDiff and totalDifficulty  -------- //
