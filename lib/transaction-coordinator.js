...
export default class TransactionCoordinator  {
  ...
  async update()
  {
            ...
            //
            setInterval(function(){this.rewardCrawl(this.mongoInterface, this.poolConfig)}.bind(this),10000)
...
//types are 'batched_payment','solution'
  ...
  async broadcastQueuedMintTransactions(mongoInterface, poolConfig){
   ...
   async broadcastQueuedBatchedPaymentTransactions(mongoInterface, poolConfig){
    ...
    console.log(' --debug-- pendingTransactions are', pendingTransactions);
    ...
    console.log(' --debug-- nextQueuedTransactionData');
    console.log(' --debug-- nextQueuedTransactionData is', nextQueuedTransactionData);
...
// ------- DETECT POOL MINING SUCCESS FOR NEW MINING REWARD SYSTEM, ONLY INCREASE BALANCE IF SOLUTION MINED ----- //
   ...
   {
    console.log('calling ewardCrawl');
  ...
  console.log('onepoolmint', onepoolmint);
     ...
     console.log('miners_challengediffs', miners_challengediffs);
      if( miners_challengediffs ){
        ...
        if( miners_challengediffs){
            ...
            let param2 = onepoolmint.epochCount - PPNLSDEPTH;
            ...
            console.log('_miningContractsData', _miningContractsData);
            // Ensure the number of elements is at most 5
            if (_miningContractsData.length > 5) {
               _miningContractsData = _miningContractsData.slice(0, PPNLSDEPTH); // Take the first PPNLSDEPTH elements
            }

            console.log('PASSED1');
            var ppnls_challenge_numbers = [];
            var pplns_totalDifficulties_BN = web3utils.toBN('0');

            for (let oneminingContract of _miningContractsData) {
              console.log('oneminingContract', oneminingContract);
              var totalDifficulty =  await  mongoInterface.findOne('totaldiff_challengenumber', {challengeNumber: oneminingContract.challengeNumber, minerport: 8081}  );
              console.log('totalDifficulty', totalDifficulty);
              if(totalDifficulty){
                let _totaldiff = totalDifficulty.totaldiff.toString();
                console.log('one _totaldiff is :: ', _totaldiff );
                pplns_totalDifficulties_BN = pplns_totalDifficulties_BN.add(web3utils.toBN(_totaldiff));
                ppnls_challenge_numbers.push(oneminingContract.challengeNumber);
              }

              console.log('pplns_totalDifficulties_BN is: ', pplns_totalDifficulties_BN.toString());
              console.log('ppnls_challenge_numbers is: :::', ppnls_challenge_numbers);
            ...
            for(let oneminer_challengetotaldiff of  miners_challengediffs){
               ...
               await PeerInterface.processAddressforChallenge( oneminer_challengetotaldiff, ppnls_challenge_numbers, pplns_totalDifficulties_BN, mongoInterface, poolConfig );
            }
    }
