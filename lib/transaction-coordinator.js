...
export default class TransactionCoordinator  {
  ...
  static _monitorMinedSolutionsRunning = false;
  static _monitorMinedPaymentsRunning = false;
  ...
  constructor(web3, poolConfig,   mongoInterface )
  {
    ...
    this._rewardCrawlRunning = false;
  ...
  async update()
  {
            ...
            setInterval(async function(){
                if (this._rewardCrawlRunning) return;
                this._rewardCrawlRunning = true;
                try {
                    await this.rewardCrawl(this.mongoInterface, this.poolConfig);
                } finally {
                    this._rewardCrawlRunning = false;
                }
            }.bind(this),60000)
  ...
  */
  ...
  {
    if (TransactionCoordinator._monitorMinedPaymentsRunning) return;
    TransactionCoordinator._monitorMinedPaymentsRunning = true;
    try{
      ...
      if( payment_txes != null && payment_txes.length > 0) {
        console.log('[PAYMENT-TRACE] monitorMinedPayments: found', payment_txes.length, 'pending batched_payment TXs | txHashes:', payment_txes.map(t => (t.txHash || 'null').substring(0,18)));
        await TransactionHelper.checkTransactionReceipts( payment_txes, web3, mongoInterface)
      }

    }catch(e){
       PoolLogger.error('monitorMinedPayments error', e)
      } finally {
        TransactionCoordinator._monitorMinedPaymentsRunning = false;
      }
  ...
  {
    if (TransactionCoordinator._monitorMinedSolutionsRunning) return;
    TransactionCoordinator._monitorMinedSolutionsRunning = true;
    try {
   ...
   {
   PoolLogger.error('monitorMinedSolutions error', e)
    } finally {
      TransactionCoordinator._monitorMinedSolutionsRunning = false;
    }
...
//types are 'batched_payment','solution'
   static async addTransactionToQueue(txType, txData,  mongoInterface, poolConfig)
  {
    ...
    var packetData = {
     ...
     PoolLogger.debug('addTransactionToQueue packetData', packetData);
  ...
  async broadcastQueuedMintTransactions(mongoInterface, poolConfig){
    ...
    var nextQueuedTransactionData = await TransactionHelper.findOneTransactionWithQuery({txType:'solution',status:'queued', "txData.challenge_number": currentchallengenumber },mongoInterface)
    ...
    nextQueuedTransactionData = await TransactionHelper.findOneTransactionWithQuery({txType:'solution',status:'queued'},mongoInterface)
    }

    var hasQueuedTransaction = (nextQueuedTransactionData != null)
    if(hasPendingTransaction) PoolLogger.debug(`Broadcasting TXS — ${pendingMintTransactions.length} pending`);
    if(hasQueuedTransaction) PoolLogger.tx('Queued solution found, broadcasting...');
   ...
   async broadcastQueuedBatchedPaymentTransactions(mongoInterface, poolConfig){
    ...
    var nextQueuedTransactionData = await TransactionHelper.findOneTransactionWithQuery({txType:'batched_payment',status:'queued'},mongoInterface)

    if(pendingTransactions.length > 0 || nextQueuedTransactionData) {
      console.log('[PAYMENT-TRACE] broadcastQueuedBatchedPayments: pending:', pendingTransactions.length, '| queued:', nextQueuedTransactionData ? 'YES' : 'NO');
    }

    if(hasPendingTransaction) PoolLogger.debug(`BatchedPayments — ${pendingTransactions.length} pending`);
    if(nextQueuedTransactionData != null) PoolLogger.tx('Queued batched payment found, broadcasting...');
    ...
    var hasQueuedTransaction = (nextQueuedTransactionData != null)
       ...
       if( hasQueuedTransaction && !hasPendingTransaction ){
          ...
          try{
              ...
              console.log('[PAYMENT-TRACE] broadcastQueuedBatchedPayments: BROADCASTING batch | uuid:', nextQueuedTransactionData.txData?.uuid?.substring(0,18), '| payments:', nextQueuedTransactionData.txData?.payments?.length);

             var successful_broadcast = await this.broadcastTransaction(nextQueuedTransactionData, mongoInterface, poolConfig);

             if(!successful_broadcast)
             {
               console.error('[PAYMENT-TRACE] broadcastQueuedBatchedPayments: broadcast FAILED')
             }
             else {
              console.log('[PAYMENT-TRACE] broadcastQueuedBatchedPayments: broadcast SUCCESS');
              PoolLogger.success('Payment TX broadcasted');
             }

          }
          catch(e)
          {
          PoolLogger.error('broadcastQueuedBatchedPayments error', e);
          }
       }
   ...
   async requeueSkippedBatchedPaymentTransactions(mongoInterface, poolConfig){
    ...
    for(let pendingTransaction of pendingTransactions){
      ...
      // Skip transactions with no txHash and no valid payments — they can never succeed
      if(!pendingTransaction.txHash && (!pendingTransaction.txData || !pendingTransaction.txData.payments || pendingTransaction.txData.payments.length === 0)){
        continue;
      }
...
// since they would go to queue and then immediatly back to skipped as their broadcast would fail
   async CheckStuckPaymentTransactions(mongoInterface, poolConfig){
    ...
    if(potentialstuckTransactions && potentialstuckTransactions.length > 0) {
      console.log('[PAYMENT-TRACE] CheckStuckPaymentTransactions: found', potentialstuckTransactions.length, 'skipped batched_payment TXs to check');
    }
    ...
    PoolLogger.error('requeueSkippedBatchedPayments error', e)
   }
   ...
   */
   ...
   async monitorStalePaymentTransactions(mongoInterface, poolConfig){
    ...
    for(let tx of staleTransactions){
      ...
      PoolLogger.warn(`Marking stale TX | ${PoolLogger.truncateHash(tx.txHash || tx._id)}`);
      PoolLogger.debug('stale tx details', tx);
      await TransactionHelper.updateOneTransactionById(tx._id, {status:'skipped'}, mongoInterface)
   ...
   if(txType=="solution"){
    ...
    isoutdated = true;
      ...
      // Skip empty batched payments — nothing to send
      if(!txData.payments || txData.payments.length === 0){
          PoolLogger.debug('Skipping empty batched payment');
          await TransactionHelper.updateOneTransactionById(transactionPacketData._id, {status:'skipped'}, mongoInterface);
          return false;
      }

          console.log('[PAYMENT-TRACE] broadcastTransaction: submitting batched_payment | uuid:', txData.uuid?.substring(0,18), '| payments:', txData.payments?.length);
          var submitResult =   await TransactionHelper.submitBatchedPayment(txData, mongoInterface, poolConfig)

          console.log('[PAYMENT-TRACE] broadcastTransaction: submitBatchedPayment result | success:', submitResult?.success, '| txHash:', submitResult?.txResult?.txHash?.substring(0,18) || 'null', '| reason:', submitResult?.reason || 'none');
          PoolLogger.debug('submitresult', submitResult)


          if(submitResult.success == true){

            tx_hash = submitResult.txResult.txHash
            PoolLogger.success('Batched payment TX broadcasted');
...
if(!isoutdated){
    ...
    if(tx_hash == null){
      ...
      console.log('[PAYMENT-TRACE] broadcastTransaction: tx_hash is NULL, setting status to SKIPPED | txType:', txType);
      await TransactionHelper.updateOneTransactionById(transactionPacketData._id, {status:'skipped'}, mongoInterface)

      return false;
    }else{
      console.log('[PAYMENT-TRACE] broadcastTransaction: tx_hash OK, setting status to PENDING | txType:', txType, '| txHash:', tx_hash.substring(0,18));
      PoolLogger.tx(`Broadcasted ${txType} | txHash=${PoolLogger.truncateHash(tx_hash)}`);
      PoolLogger.debug('broadcasted transaction details', { tx_hash, txType, txData });
    ...
    PoolLogger.info(`TX outdated | challenge changed | ${PoolLogger.truncateHash(txData.challenge_number)}`);
    ...
    async getTransactionStatistics(mongoInterface){
     ...
     async requestTransactionData(tx_hash)
     {
       ...
       try{
        ...
        {
          PoolLogger.debug(`Failed to get tx data for ${PoolLogger.truncateHash(tx_hash)}`);
          return null;
        }
...
// ------- DETECT POOL MINING SUCCESS FOR NEW MINING REWARD SYSTEM, ONLY INCREASE BALANCE IF SOLUTION MINED ----- //
      ...
      async detectPoolMiningSuccess (tokenContract, web3,  mongoInterface, poolConfig )
     {
       if(typeof tokenContract == 'undefined'){
         PoolLogger.warn('Could not detect mining success — tokenContract undefined')
         return
       }
       ...
       .catch(err => PoolLogger.error('detectPoolMiningSuccess error', err));
     }
       ...
       try{
        ...
        var max_gas_cost = 700624;
         ...
         try{
           ...
           if( estimatedGasCost > max_gas_cost){
             PoolLogger.error('SendEtiToBatchContract gas estimate too high')
             return;
           }
         }catch(error){
           ...
           console.error('SendEtiToBatchContractCrawl estimatedGasCostError:', error.message || error);
        ...
        let mintingGasPriceBoost = 0;
         ...
         try{
           ...
           LoggingHelper.appendLog( ['estimatedGasCostError', error], LoggingHelper.TYPECODES.ERROR, mongoInterface)
           console.error('Mint tx rejected:', error.message || error);
          ...
          try{
            ...
            console.error('SendEtiToBatchContract estimatedGasCostError2:', error.message || error);
            // We abandon:
