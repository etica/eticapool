...
//types are 'batched_payment','solution'
  ...
  async broadcastQueuedMintTransactions(mongoInterface, poolConfig){
    ...
    console.log('broadcastQueuedMintTransactions stage a1 --> ');
    ...
    console.log('broadcastQueuedMintTransactions stage a1 hasPendingTransaction: --> ', hasPendingTransaction);
       ...
       if( hasQueuedTransaction && !hasPendingTransaction ){
          ...
          try{
            ...
            console.log('broadcastQueuedMintTransactions stage b1 --> ');
             ...
             console.log('broadcastQueuedMintTransactions stage b2 --> ');
             ...
             {
              console.log('broadcastQueuedMintTransactions stage b3 --> ');
        ...
        console.log('broadcastQueuedMintTransactions stage b5 --> ');
       }
...
// since they would go to queue and then immediatly back to skipped as their broadcast would fail
  ...
  // Mark solution txs waiting for broadcast as outdated to avoid them from broadcasting and preventing uptodate solutions to be submited:
   ...
   // Deletes pending shares waiting for processing to avoid them from overwhelming shares processing as the pool randomxBlob has now changed and they will be rejected:
   async OutdatePendingShares(_challengeNumber, mongoInterface){

    await mongoInterface.deleteMany('queued_shares_list', {'challengeNumber': _challengeNumber } );
...
if(!isoutdated){
    ...
    await this.OutdatePendingShares(txData.challenge_number, mongoInterface); // outdates all pending shares with this outdated challenge number
   ...
   async rewardCrawl( mongoInterface, poolConfig )
   {
    ...
    for(let onepoolmint of unprocessedPoolmints)
    {
     ...
     for(let oneminerAddress of uniqueMiners){
      ...
      let existingChallengediff = await mongoInterface.findOne('miner_challengediff', {minerEthAddress: oneminerAddress, challengeNumber: _miningContractData.challengeNumber, minerport: 8081});
