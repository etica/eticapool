...
export default class TransactionCoordinator  {
  ...
  async update()
  {
            ...
            //
            setInterval(function(){this.rewardCrawl(this.mongoInterface, this.poolConfig)}.bind(this),12000)
...
// ------- DETECT POOL MINING SUCCESS FOR NEW MINING REWARD SYSTEM, ONLY INCREASE BALANCE IF SOLUTION MINED ----- //
      ...
      async detectPoolMiningSuccess (tokenContract, web3,  mongoInterface, poolConfig )
     {
       ...
       console.log('--  SERACHING DETECTING POOL MINTS ---');
       let _searchrange = 3000; // range search of number of blocks to look for a past mining pool success
       ...
       events.forEach(onevent => {
            console.log('---------> ONEEVENT mint is: ', onevent);
...
// ------- DETECT POOL MINING SUCCESS FOR NEW MINING REWARD SYSTEM, ONLY INCREASE BALANCE IF SOLUTION MINED ----- //
   ...
   {
     console.log('rewardCrawl called !!!');
    var unprocessedPoolmints =  await  mongoInterface.findAll('pool_mints', {poolstatus: 1}  );
    console.log('unprocessedPoolmints are', unprocessedPoolmints);
    for(let onepoolmint of unprocessedPoolmints)
    {
      console.log('rewardCrawling onepoolmint', onepoolmint);
  // get poolmint's pendingshare thanks to allminingContractsthat has both fields epochCount and challengeNumber to make the link:
     ...
     console.log('_miningContract is', _miningContractData);
      ...
      if( miners_challengediffs ){
        ...
        if( miners_challengediffs){
            for(let oneminer_challengetotaldiff of  miners_challengediffs){
               ...
               await PeerInterface.processAddressforChallenge( oneminer_challengetotaldiff, mongoInterface, poolConfig );
  }
}
  ...
  await mongoInterface.updateOne('pool_mints', {_id: onepoolmint._id}, {poolstatus: 2});
      ...
      console.log('no miners_challengediffs found for this poolmint');
      ...
      console.log('!!! Warning no mining contract found !!! Cant process the mint need to be handle manually or add the mining contract');
