...
export default class StatsInterface {
  ...
  async  update()
  {
    ...
    setInterval(function(){ StatsInterface.CalculateCurrentStatsPayment(this.mongoInterface,this.poolConfig)}.bind(this) , 3600*1000) // Every hour
  } 
  ...
  static async CalculateCurrentStatsPayment(mongoInterface, poolConfig)
    { 
      ...
      for(let minerData of minerList) //reward each miner
      {
        ...
        console.log('inside CalculateCurrentStatsPayment()');
        console.log('minerAddress is: ', minerAddress);
