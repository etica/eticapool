...
handleUpdateForAllWorkers(updatedChallenge) {
  this.workers.forEach((worker) => {
      worker.handleUpdate(updatedChallenge);
  });
}
...
async startSratumServer(){
  ...
  const difficulty = PeerHelper.getPoolMinimumShareDifficultyHard(poolConfig);
  poolConfig.minimumTarget = PeerHelper.getTargetFromDifficulty(difficulty);
