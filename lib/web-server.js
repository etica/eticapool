...
async CheckChallengeNumberForWorkers(){
  console.log('just before webserver.js GeneralEventEmitterHandler.getInstance()');
  GeneralEventEmitterHandler.getInstance().on('newChallenge', async (miningContractData) => {
    console.log('-a- a- a- -a - a- a- a- -a - a- a NEW CHALLENGE miningContractData received');
    console.log('-a- a- a- -a - a- a- a- -a - a- a NEW CHALLENGE miningContractData received: ', miningContractData);
    console.log('-a- a- a- -a - a- a- a- -a - a- a this.challengeNumber: ', this.challengeNumber);
    console.log('just after webserver.js GeneralEventEmitterHandler.getInstance()');
    if (miningContractData != null && miningContractData.challengeNumber && this.challengeNumber != miningContractData.challengeNumber){
      console.log('-a- a- a- -a - a- a- a- -a - a- a NEW CHALLENGE IDENTIFIED');
      console.log('-a- a- a- -a - a- a- a- -a - a- a miningContractData.challengeNumber: ', miningContractData.challengeNumber);
      this.challengeNumber = miningContractData.challengeNumber;
...
console.log('startSratumServer stage II called');
              ...
              console.log(`New connection on port ${socket.localPort}`);
                socket.once("data", (data) => {
                  ...
                  try {
                        ...
                        //console.log('startSratumServer poolConfig: ', poolConfig);
                        
                        var startingdifficulty = PeerHelper.getPoolMinimumShareDifficultyHard(poolConfig);
                        if(socket.localPort == 3333){
                          startingdifficulty = 50000;
                        }
                        if(socket.localPort == 5555){
                          startingdifficulty = 100000;
                        }
                        if(socket.localPort == 7777){
                          startingdifficulty = 210000;
                        }
                        console.log('startingdifficulty is: ', startingdifficulty);
                        ...
                        console.error('Error handling Stratum connection:', error);
                        // Handle the error gracefully without crashing the server
                      }
            ...
            //console.log('stratumserver is: ', stratumserver);
              ...
              stratumserver.listen(5555, function () {
                console.log('Stratum Server serving on 5555!')
            });

              stratumserver.listen(7777, function () {
                console.log('Stratum Server serving on 7777!')
            });
