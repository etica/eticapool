...
export default class RedisInterface {
    ...
    async init(redisUrl) {
        ...
        // Wait for ready (clean up listeners to avoid leaks)
        await new Promise((resolve, reject) => {
            const onReady = () => { this.client.removeListener('error', onError); resolve(); };
            const onError = (err) => { this.client.removeListener('ready', onReady); reject(err); };
