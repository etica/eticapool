...
export default class ResetWorkersAddresses  {
  ...
  async ResetWorkersBalances(mongoInterface) {
      ...
      const rawAmountToFormatted = Web3ApiHelper.rawAmountToFormatted(actual_total_coins_owed, TOKEN_DECIMALS );
      const rawAmountToFormattedAll = Web3ApiHelper.rawAmountToFormatted(total_coins_owed, TOKEN_DECIMALS );

      console.log('total_coins_owed: ', rawAmountToFormattedAll, 'ETI');
      console.log('actual_total_coins_owed: ', rawAmountToFormatted, 'ETI');


      console.log('---------- Total amount of ETI awaiting payouts for addresses that used worker names calculated. ---------');
      
      console.log('------------------------------------------------------------------------------------------------------');
      console.log('------------ Total pool unpaid amount (includes addresses with or without worker names) is: ');
      console.log(rawAmountToFormattedAll, 'ETI');
      console.log('------------ Unpaid amount to addresses using worker names is: ');
      console.log(rawAmountToFormatted, 'ETI');
      console.log('------------------------------------------------------------------------------------------------------');
      console.log('Check your pool has enough liquidity to honour the total amount of: ');
      console.log(rawAmountToFormattedAll, 'ETI');
      console.log('If not confirm next step to reset and cancel balances of addresses that used worker names.');
      ...
      console.log("Warning: Continuing will reset balances to 0 for all addresses with worker names.");
