...
// TODO: mongodb driver v3.6 is EOL. Upgrade to v6+ requires rewriting
// all callback-based methods to async/await (breaking API change).
...
var url = process.env.MONGODB_URI || "mongodb://eticapool:eticapool_mongo_default_change_me@mongodb:27017";
var dbo;
...
export default class MongoInterface  {
    ...
    async init( dbName  )
    {
      ...
      var database = await new Promise(function(resolve, reject) {
            mongoClient.connect(url, { useUnifiedTopology: true, retryWrites: true, writeConcern: { w: 1, j: true } }, function(err, db) {
              if (err) { reject(err); return; }
                dbo = db.db( dbName, { writeConcern: { w: 1, j: true } } );
    ...
    async createCollectionUniqueIndexes()
    {
      ...
      await this.createIndexOnCollection('balance_payments', 'batchedPaymentUuid')
      ...
      await this.createChartLinedataIndex()

      await this.createAuthNoncesIndex()
    ...
    {
      await dbo.collection(collectionName).createIndex( { [`${indexColumnName}`]: 1 }, { unique: false } )
    }

    async createUniqueIndexOnCollection(collectionName, indexColumnName)
    {
      await dbo.collection(collectionName).createIndex( { [`${indexColumnName}`]: 1 }, { unique: true } )
    }

    async createDualIndexOnCollection(collectionName, indexColumnNameA, indexColumnNameB)
    {
      await dbo.collection(collectionName).createIndex( { [`${indexColumnNameA}`]: 1,  [`${indexColumnNameB}`]: 1 }, { unique: false } )
    }

    async createTripleIndexOnCollection(collectionName, indexColumnNameA, indexColumnNameB, indexColumnNameC)
    {
      await dbo.collection(collectionName).createIndex( { [`${indexColumnNameA}`]: 1,  [`${indexColumnNameB}`]: 1 , [`${indexColumnNameC}`]: 1}, { unique: false } )
    }

    async createUniqueTripleIndexOnCollection(collectionName, indexColumnNameA, indexColumnNameB, indexColumnNameC)
    {
      await dbo.collection(collectionName).createIndex( { [`${indexColumnNameA}`]: 1,  [`${indexColumnNameB}`]: 1 , [`${indexColumnNameC}`]: 1}, { unique: true } )
    }

    async createUniqueDualIndexOnCollection(collectionName, indexColumnNameA, indexColumnNameB)
    {
      await dbo.collection(collectionName).createIndex( { [`${indexColumnNameA}`]: 1,  [`${indexColumnNameB}`]: 1 }, { unique: true } )
    }

    async createAuthNoncesIndex()
    {
      await dbo.collection('auth_nonces').createIndex(
        { createdAt: 1 },
        { expireAfterSeconds: 300 }
      );
    }

    async createChartLinedataIndex()
    {
      await dbo.collection('chart_linedata').createIndex(
        { name: 1 },
        { unique: true }
      );
    }
