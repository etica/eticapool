...
export default class RequeueReverted  {
  ...
  async check_miner_diff_challenge_rewards(_challengeNumber){
    ...
    var totalDifficulty =  await  mongoInterface.findOne('totaldiff_challengenumber', {challengeNumber: _challengeNumber, minerport: 8888}  );
    ...
    for(let oneminer_challengediff of miners_challengediffs){
      ...
      let tokenRewardAmt = await PeerHelper.getTokenRewardForShareOfDifficulty(shareCredits, totalDifficulty.totaldiff, 8888, poolConfig, mongoInterface)
   ...
   async get_miner_challengediffs(_challengeNumber){
    ...
    let mongoInterface = this.mongoInterface 
      ...
      let miners_challengediffs = await mongoInterface.findAll('miner_challengediff', {challengeNumber: _challengeNumber, minerport: 8888});
      var miners_ratio = 0;
    ...
    { 
      ...
      var actual_total_num_coins_owed = 0;
         ...
         {  
             ...
             if(num_coins_owed > min_balance_for_transfer){
              ...
              && web3utils.isAddress( minerAddress )){
                console.log('passed1')
                actual_total_next_coins_batchs = actual_total_next_coins_batchs + num_coins_owed;
              }
             ...
             if(web3utils.isAddress( minerAddress )){
                console.log('passed2')
                actual_total_num_coins_owed = actual_total_num_coins_owed + num_coins_owed;
              }
      ...
      console.log('actual_total_num_coins_owed is: ', actual_total_num_coins_owed); 
