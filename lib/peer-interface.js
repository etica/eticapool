   ...
   static async handlePeerShareSubmit(shareData ,mongoInterface, poolConfig)
   {
    ...
    let extraNonce = shareData.extraNonce
     ...
     let poolEpochCount = await TokenDataHelper.getEpochCount(mongoInterface);
      ...
      var minerBlob = randomxHelper.setReservedHashOnBlob(poolRandomxBlob, poolEthAddress, poolChallengeNumber, extraNonce);
      minerBlob = randomxHelper.convertToRawWithout0x(minerBlob);
     ...
     const isValid = randomx.VerifyEticaRandomXNonce(minerBlob, nonceBuffer, targetBuffer, seedHash, randomxhashRaw);
...
//   NEW SYSTEM REWARD BASED ON ADDRESSES INSTEAD OF SHARES, REMOVE MINER_SHARES ONLY USES MINER_PENDINGSHARES   //
   ...
   static async  prehandleValidShare( nonce, randomxhash, randomx_blob, randomx_seedhash, claimedtarget, minerEthAddress,digest,difficulty, shareIsASolution, minerport, mongoInterface, poolConfig )
   {
...
await mongoInterface.insertOne('miner_challengediff', newminerchallengediff);
  ...
  await mongoInterface.updateOne('miner_challengediff', {minerEthAddress: minerEthAddress.toString().toLowerCase(), challengeNumber: challengeNumber, minerport: minerport},  {totaldiff: _newminertotaldiff});
        ...
        if( shareIsASolution )
        {
          ...
          await TokenInterface.queueMiningSolution( nonce,minerEthAddress,digest,challengeNumber, randomx_blob, randomx_seedhash, randomxhash, claimedtarget, mongoInterface, poolConfig );
        }else{
            // nothing 
        }
