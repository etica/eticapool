...
export default class PeerInterface  {
  ...
  constructor(mongoInterface, poolConfig, redisInterface){
  ...
  _emitMinerShareUpdate(shareData, result) {
    if (!this.io || !shareData || !shareData.minerEthAddress) return;
    if (!result || !result.success) return;
    var addr = shareData.minerEthAddress.toString().toLowerCase();
    var addrBlockchain = addr.substr(0, 42);
    // Extract worker name (portion after the '.') or null for bare address mining
    var workerName = addr.length > 42 ? addr.substring(43) : null;
    this.io.to('miner:' + addrBlockchain).emit('minerShareUpdate', {
      minerEthAddress: addr,
      addressBlockchain: addrBlockchain,
      workerName: workerName,
      newShare: {
        block: shareData.block,
        difficulty: shareData.difficulty,
        challengeNumber: shareData.challengeNumber,
        entryport: shareData.entryport,
        minerEthAddress: addr,
  ...
  async update()
  {
    ...
    setInterval(function(){PeerHelper.calculateLast24hEarnings(this.mongoInterface)}.bind(this), 360000) // 6 minutes
    setInterval(function(){PeerHelper.resetLast24hNoEarnings(this.mongoInterface)}.bind(this), 1800000) // 30 minutes
  ...
  {
    if (this.redisInterface) {
      // Redis Streams consumer: blocking read, batch processing, crash recovery
      await this.processQueuedSharesRedis(mongoInterface, poolConfig);
    } else {
      // Fallback: MongoDB polling (legacy)
      await this.processQueuedSharesMongo(mongoInterface, poolConfig);
    }
  }

  async processQueuedSharesRedis(mongoInterface, poolConfig)
  {
    const consumerName = `processor-${process.pid}`;
    const RECLAIM_INTERVAL_MS = 60000; // Reclaim stale shares every 60 seconds
    let lastReclaimTime = 0;

    // Claim any stale shares from crashed consumers on startup and drain PEL
    try {
      await this.redisInterface.claimStaleShares(consumerName);
      await this._drainPendingShares(consumerName, mongoInterface, poolConfig);
    } catch (err) {
      console.error('processQueuedSharesRedis startup claim error:', err);
    }
    lastReclaimTime = Date.now();

    while (!this._shuttingDown) {
      try {
        // Periodically reclaim stale shares from crashed consumers and drain PEL
        ...
        const messages = await this.redisInterface.readShares(consumerName, 10, 2000);
        if (!messages || messages.length === 0) continue;

        for (const msg of messages) {
          try {
            const shareData = JSON.parse(msg.data);
            var result = await PeerInterface.handlePeerShareSubmit(shareData, mongoInterface, poolConfig);
            this._emitMinerShareUpdate(shareData, result);
            // Only ack after successful processing; failed shares stay pending for reclaim
            await this.redisInterface.ackShare(msg.id);
          } catch (err) {
            if (err instanceof SyntaxError) {
              // Unparseable JSON - ack to prevent infinite retry of unrecoverable message
              console.error('Unparseable share data, discarding:', err.message);
              await this.redisInterface.ackShare(msg.id);
            } else {
              // Processing error (e.g., transient DB failure) - don't ack, will be reclaimed
              console.error('handle share error (will retry):', err);
              LoggingHelper.appendLog(['handle share error', err], LoggingHelper.TYPECODES.ERROR, mongoInterface);
            }
          }
        }
      } catch (err) {
        console.error('processQueuedSharesRedis error:', err);
  ...
  // Drain all pending (claimed but unacked) messages from this consumer's PEL.
  // XREADGROUP with '0' returns messages in the PEL, not new ones.
  async _drainPendingShares(consumerName, mongoInterface, poolConfig) {
    let drained = 0;
    while (true) {
      const pending = await this.redisInterface.readPendingShares(consumerName, 10);
      if (!pending || pending.length === 0) break;

      for (const msg of pending) {
        try {
          const shareData = JSON.parse(msg.data);
          await PeerInterface.handlePeerShareSubmit(shareData, mongoInterface, poolConfig);
          await this.redisInterface.ackShare(msg.id);
          drained++;
        } catch (err) {
          if (err instanceof SyntaxError) {
            console.error('Unparseable pending share, discarding:', err.message);
            await this.redisInterface.ackShare(msg.id);
            drained++;
          } else {
            console.error('Error processing pending share (will retry later):', err);
            return drained; // Stop draining on transient error; will retry next reclaim cycle
          }
        }
      }
    }
    if (drained > 0) {
      console.log(`Drained ${drained} pending shares from PEL for ${consumerName}`);
    }
    return drained;
  }

  async processQueuedSharesMongo(mongoInterface, poolConfig)
  {
    ...
    try {
      ...
      if(typeof shareData != 'undefined' && shareData != null)
      {
        ...
        try{
          ...
          this._emitMinerShareUpdate(shareData, response);
          }catch(err)
          {
            console.error(err)
            LoggingHelper.appendLog(['handle share error',err], LoggingHelper.TYPECODES.ERROR, mongoInterface)
      ...
      console.error('processQueuedShares error:', err)
    }

    var pollDelay = shareFound ? 0 : 500;
    setTimeout(function(){this.processQueuedSharesMongo(mongoInterface,poolConfig)}.bind(this), pollDelay)
  } 
   ...
   */
   ...
   static async handlePeerShareSubmit(shareData ,mongoInterface, poolConfig)
   {
    ...
    let extraNonce = shareData.extraNonce
     ...
     // Single DB read instead of 4 separate calls that each read the same miningContractData document
     let recentMiningContractData = await TokenDataHelper.getRecentMiningContractData(mongoInterface);
     let poolChallengeNumber = recentMiningContractData.challengeNumber;
     let poolRandomxBlob = recentMiningContractData.randomxBlob;
     let poolConfigRandomxSeedhash = recentMiningContractData.randomxSeedhash;
     let poolEpochCount = recentMiningContractData.epochCount;


      // Early reject if the submitted blob doesn't match the current pool blob
      ...
      var randomxhashBuffer;

      if (shareData.hashVerified === true) {
        // Pass 1 (worker.js) already computed and verified the RandomX hash.
        // The blob matches the pool blob (checked above), so the hash is trustworthy.
        // Skip the expensive multiHashing.randomx() call.
        randomxhashBuffer = Buffer.from(randomxhashRaw, 'hex');
      } else {
        // Backwards compatible: full hash verification for shares without hashVerified flag
        ...
        const blobWithNonce = Buffer.from(newVerifyminerBlob, 'hex');
        ...
        // Convert the expected randomxhashRaw hex string to a Buffer
        randomxhashBuffer = Buffer.from(randomxhashRaw, 'hex');

        // Compare calculated hash with expected hash
        const hashesMatch = calculatedHash.equals(randomxhashBuffer);

        if (!hashesMatch) {
            console.log("Calculated hash does not match expected hash");
            return { success: false, message: "Calculated hash does not match expected hash" };
        }
      }
    ...
    const isClaimedTarget = reversedRandomxhashBN.lte(claimedtargetBN);

    if (!isClaimedTarget) {
      console.log("Calculated hash does not meet claimed target");
      return { success: false, message: "Calculated hash does not meet claimed target" };
    }

    // --> If reach here, hash verified and isClaimedTarget true means SHARE IS VALID
     ...
     miningTarget = web3utils.toBN( recentMiningContractData.miningTarget ) ;
    ...
    //console.log('SHARE IS A NETWORK SOLUTION IS: ', shareIsASolution)
      ...
      const verifiedminerBlob = randomxHelper.convertAdd0x(newVerifyminerBlob.toString('hex')); //adds 0x to fit pool db storage standards
        ...
        return await this.prehandleValidShare( nonce,
                                            ...
                                            digest,
                                            difficulty,
                                            shareIsASolution, minerport, entryport, mongoInterface, poolConfig, poolChallengeNumber );
...
//   NEW SYSTEM REWARD BASED ON ADDRESSES INSTEAD OF SHARES, REMOVE MINER_SHARES ONLY USES MINER_PENDINGSHARES   //
   ...
   static async  prehandleValidShare( nonce, randomxhash, randomx_blob, randomx_seedhash, claimedtarget, blobWithextraNonce, extraNonce, minerEthAddress,digest,difficulty, shareIsASolution, minerport, entryport, mongoInterface, poolConfig, poolChallengeNumber )
   {
      ...
      { 
       ...
       if(timeToFindShare >= 5 || isfirstsolution)
        ...
        // Use poolChallengeNumber passed from handlePeerShareSubmit instead of re-reading from DB
        var challengeNumber = poolChallengeNumber;
...
// NOTE: totaldiff fields in both totaldiff_challengenumber and miner_challengediff are stored
// as BN strings (e.g. "123456789"), not as numbers. MongoDB $inc requires numeric types, so we
// cannot use atomic $inc here. We must read-compute-write using BN arithmetic to preserve
// arbitrary-precision integer addition on these string fields.

let _td = await mongoInterface.findOne('totaldiff_challengenumber', {challengeNumber: challengeNumber, minerport: minerport});
if (!_td) {
  console.log('totaldiff_challengenumber not found for challenge', challengeNumber, 'port', minerport, '- skipping share');
...
let minerEthAddressLower = minerEthAddress.toString().toLowerCase();
let minerChallengediffQuery = {minerEthAddress: minerEthAddressLower, challengeNumber: challengeNumber, minerport: minerport};

// Run insertOne for pending share in parallel with miner_challengediff read since they are independent
let [, _miner_challengediff] = await Promise.all([
  mongoInterface.insertOne("miner_pendingshares", shareData),
  mongoInterface.findOne('miner_challengediff', minerChallengediffQuery)
]);

if(_miner_challengediff == null){
  var newminerchallengediff =  {
    minerEthAddress: minerEthAddressLower,
...
await mongoInterface.insertOne('miner_challengediff', newminerchallengediff);
  ...
  await mongoInterface.updateOne('miner_challengediff', minerChallengediffQuery,  {totaldiff: _newminertotaldiff});
}
  ...
  //   NEW SYSTEM REWARD BASED ON ADDRESSES INSTEAD OF SHARES, REMOVE MINER_SHARES ONLY USES MINER_PENDINGSHARES   //
     ...
     async initJSONRPCServerHighDiff()
     {
       ...
       let redisInterface = this.redisInterface
       ///var self = this;
       ...
       // ------------  PoolSuspended   ------------  //
        ...
        // ------- PoolSuspended -------- //
          ...
          if(!poolSuspended && validJSONSubmit){
            if (redisInterface) {
              try {
                await redisInterface.addShare(shareData);
              } catch (redisErr) {
                console.error('Redis addShare failed in JSONRPC, falling back to MongoDB:', redisErr.message);
                await mongoInterface.insertOne('queued_shares_list', shareData);
              }
            } else {
              await mongoInterface.insertOne('queued_shares_list', shareData);
            }
          ...
          callback(null,  validJSONSubmit );
           ...
           getMinerData: async function(args, callback) {
             ...
             if(web3utils.isAddress(minerAddressWithoutWorkerName.toString()) ){
                 minerData = await PeerHelper.getMinerData(minerEthAddress, mongoInterface);
             }
         ...
         getMinerDataWithWorkers: async function(args, callback) {
            ...
            if(web3utils.isAddress(minerAddressWithoutWorkerName.toString()) ){
                minerData = await PeerHelper.getMinerDataWithWorkers(minerEthAddress, mongoInterface);
            }
