...
await mongoInterface.insertOne('miner_challengediff', newminerchallengediff);
  ...
  await mongoInterface.updateOne('miner_challengediff', {minerEthAddress: minerEthAddress.toString().toLowerCase(), challengeNumber: challengeNumber, minerport: minerport},  {totaldiff: _newminertotaldiff});
   ...
   // _ppnls_challenge_numbers array containing challengenumbers of difficulties to use for ppnls sharecredits calculation
   // _pplns_totalDifficulties_BN sum of pool Total shares difficulties for each challenge number
   static async  processAddressforChallenge( _minerChallengeDiff, _ppnls_challenge_numbers, _pplns_totalDifficulties_BN, mongoInterface, poolConfig )
   {
     ...
     let existingChallengediff = await mongoInterface.findOne('miner_challengediff', {minerEthAddress: _minerChallengeDiff.minerEthAddress, challengeNumber: _minerChallengeDiff.challengeNumber, status:1, minerport: _minerChallengeDiff.minerport});
        ...
        //var shareCredits = Math.floor(_minerChallengeDiff.totaldiff);
          // This function increases field MinerData.ShareCredits but note the var ShareCredits is used instead of field MinerData.ShareCredits, this field is not used at all in the app
            ...
            // Get last X miner miner_challengediff

            // var pplns_shareCredits : Add X miner_challengediff.totaldiffs 
            ...
            for(let one_ppnls_challenge_numbers of _ppnls_challenge_numbers){

              console.log('one_ppnls_challenge_numbers is', one_ppnls_challenge_numbers);

              var oneChallengediff = await mongoInterface.findOne('miner_challengediff', {minerEthAddress: _minerChallengeDiff.minerEthAddress, challengeNumber: one_ppnls_challenge_numbers, minerport: _minerChallengeDiff.minerport});
              
              if(oneChallengediff){
                console.log('oneChallengediff is: ', oneChallengediff.totaldiff);
                ...
                console.log('no shares for this challenge number');
            ...
            console.log('total shareCredits is :', shareCredits);
            ...
            if(shareCredits > 0){
              ...
              if(poolAccountBalances.paymentsAccountBalances['token'] > poolsecuritybalance){  
                ...
                if(_pplns_totalDifficulties_BN.gt(web3utils.toBN('0'))){
                  console.log("_pplns_totalDifficulties_BN.gt(web3utils.toBN('0')) is true")
                  ...
                  console.log('_totaldiff', _totaldiff);
                   ...
                   await PeerHelper.awardTokensBalanceForShares( _minerChallengeDiff.minerEthAddress, shareCredits, _totaldiff, _minerChallengeDiff.minerport, poolConfig, mongoInterface )
                   // ---->  increase token balance  < ----- //
                }
          ...
          await mongoInterface.updateOne('miner_challengediff', {_id: _minerChallengeDiff._id, minerport: _minerChallengeDiff.minerport},  {status:3}); // Rq: this case should never happen but could create script to check for status 3

          return {success: false, message: "No shares for this ppnls period."} // no shareCredits
      }
