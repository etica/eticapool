...
await mongoInterface.insertOne('miner_challengediff', newminerchallengediff);
  ...
  await mongoInterface.updateOne('miner_challengediff', {minerEthAddress: minerEthAddress.toString().toLowerCase(), challengeNumber: challengeNumber, minerport: minerport},  {totaldiff: _newminertotaldiff});
   ...
   static async  processAddressforChallenge( _minerChallengeDiff, mongoInterface, poolConfig )
   {
     // Should arrive here for process only if status is still 1, make a check for security but should definitely never happen:
     console.log('processAddressforChallenge _minerChallengeDiff.minerEthAddress is: ', _minerChallengeDiff.minerEthAddress);
     ...
     console.log('passed 1');
          ...
          // This function increases field MinerData.ShareCredits but note the var ShareCredits is used instead of field MinerData.ShareCredits, this field is not used at all in the app
            ...
            if(totalDifficulty != null){
              ...
              if(poolAccountBalances.paymentsAccountBalances['token'] > poolsecuritybalance){  
                ...
                if(currentchallengenumber == _minerChallengeDiff.challengeNumber){
                  console.log('currentchallengenumber == _minerChallengeDiff.challengeNumber is: true');
                ...
                if(totalDifficulty.totaldiff > 0){
                   ...
                   await PeerHelper.awardTokensBalanceForShares( _minerChallengeDiff.minerEthAddress, shareCredits, totalDifficulty.totaldiff, _minerChallengeDiff.minerport, poolConfig, mongoInterface )
                   // ---->  increase token balance  < ----- //
                }

                  console.log('currentchallengenumber == _minerChallengeDiff.challengeNumber is: false');
                  return {success: true, message: "New share credited successfully with new reward system"}

                }

                else {
                  await mongoInterface.updateOne('miner_challengediff', {_id: _minerChallengeDiff._id, minerport: _minerChallengeDiff.minerport},  {status:6});
                  return {success: true, message: "New share skipped because challenge number is now outdated"}
                }
        ...
        console.log('diff  not found for This share digest');

          await mongoInterface.updateOne('miner_challengediff', {_id: _minerChallengeDiff._id, minerport: _minerChallengeDiff.minerport},  {status:3}); // in case pb with totaldifficulty prevented from making the reward need to check manually. Rq: this case should never happen but could create script to check for status 3

        return {success: false, message: "This share digest was already processed"} // totalDifficulty not found
      }
