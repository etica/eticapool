   ...
   */
   ...
   static async handlePeerShareSubmit(shareData ,mongoInterface, poolConfig)
   {
    ...
    let extraNonce = shareData.extraNonce
     ...
     if( difficulty == null || nonce == null || minerEthAddress == null || challengeNumber == null || 
      digest == null || randomxhash == null || randomx_blob == null || 
      randomx_seedhash == null || claimedtarget == null ) {
      console.log('---------> SHARE REJECTED - Missing required fields');
      console.log('----------> Miner Address:', minerEthAddress);
      return;
  }
    ...
    if (!hashesMatch) {
      console.log('---------> SHARE REJECTED - Hash Mismatch');
    console.log('----------> Miner Address:', minerEthAddress);
    ...
    if (!isClaimedTarget) {
      console.log('---------> SHARE REJECTED - Target Not Met');
    console.log('----------> Miner Address:', minerEthAddress);
    ...
    // --> If reach here, hashesMatch and isClaimedTarget true means SHARE IS VALID
     ...
     if (randomx_blob != poolRandomxBlob) {
      console.log('---------> SHARE REJECTED - Invalid Blob');
    console.log('----------> Miner Address:', minerEthAddress);
    console.log('----------> Submitted randomx_blob does not match pool randomx_blob');
    ...
    if (claimedtargetBN.gt(minShareTarget)) {
      console.log('---------> SHARE REJECTED - Difficulty Too Low');
    console.log('----------> Miner Address:', minerEthAddress);
    console.log('----------> Share difficulty:', difficulty);
      return { success: false, message: "Share difficulty too low" };
    } 
    
    console.log('---------> SHARE FULLY VALIDATED');
console.log('----------> Miner Address:', shareData.minerEthAddress);
console.log('----------> Difficulty:', shareData.difficulty);
