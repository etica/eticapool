...
export default class PeerInterface  {
  ...
  async listenForStratumServer()
  {

        //this.initJSONRPCServer();
        console.log('called listenForStratumServer from index')
        this.initStratumServerHighDiff();
...
/* 
   ...
   */
   ...
   static async handlePeerShareSubmit(shareData ,mongoInterface, poolConfig)
   {
    ...
    // minShareTarget is the var that truly filters miners hashrate (due to shares accepted) and thus their rewards, to let miners
    // with low hashrate mine eticas
...
/*
  async initJSONRPCServer()
     ...
     async initStratumServerHighDiff()
     {

      let mongoInterface = this.mongoInterface 
      let poolConfig = this.poolConfig 

      console.log('listening Stratum Server localhost:3333')


// Create a new Stratum server
const stratumServer = stratum.Server.create();
console.log('stratumServer: ', stratumServer)

// Handle mining-related requests from miners
stratumServer.on('mining', async (req, deferred, socket) => {

  console.log('stratumServer on mining');
  console.log('stratumServer req', req);
  ...
  switch (method) {
    case 'getPoolProtocolVersion':
      deferred.resolve('Your Pool Protocol Version Here');
      break;

    case 'getPoolStatus':

      console.log('getPoolStatus called')
      const poolSuspended = await PoolStatsHelper.poolMintingIsSuspended(poolConfig,mongoInterface);
      
      deferred.resolve({ poolIsSuspended: poolSuspended });
      break;

    case 'getPoolEthAddress':

      const poolAddress = await PeerHelper.getPoolEthAddress(poolConfig).toString();
      
      deferred.resolve(poolAddress);
      break;


    case 'getMinimumShareDifficulty':

      const _mindifficultyshare = await PeerHelper.getPoolMinimumShareDifficultyHard(poolConfig);
      
      deferred.resolve(_mindifficultyshare);
      break;

    case 'getMinimumShareTarget':

      const _minsharetarget = await PeerHelper.getPoolMinimumShareTargetHard(poolConfig);
      
      deferred.resolve(_minsharetarget);
      break;

    case 'getChallengeNumber':
      const challengenumber = await TokenDataHelper.getChallengeNumber( mongoInterface ) 
      ...
      deferred.resolve(challengenumber);
      break;


    case 'getMinerData':

     const [minerEthAddress_getMinerData] = params;

     var minerData = null;

     if(web3utils.isAddress(minerEthAddress_getMinerData.toString()) ){
        minerData = await PeerHelper.getMinerData(minerEthAddress_getMinerData, mongoInterface);
     }else{
      console.log('getMinerData error: not a valid address')
     }
        
        deferred.resolve(JSON.stringify( minerData ));
        break;


    case 'getAllMinerData':
    ...
    // Handle other RPC methods similarly

    case 'submitShare':
      const [nonce, minerEthAddress, digest, difficulty, challenge_number, custom_vardiff_used] = params;
      ...
      // cool mining pool
       ...
       // ------------  PoolSuspended   ------------  //
        ...
        // ------- PoolSuspended -------- //
          ...
          if(!_poolSuspended && validJSONSubmit){
            var response = await mongoInterface.insertOne('queued_shares_list', shareData )            
      ...
      deferred.resolve(validJSONSubmit);
      break;
...
// Handle RPC events from outside (blocknotify, etc.)
stratumServer.on('rpc', (name, args, connection, deferred) => {
  // Implement handling of RPC events (blocknotify, etc.)
  // Your logic here
});

// Start the Stratum server
stratumServer.listen().then(msg => {
  console.log('stratumServer.listen() revealed');
  console.log('stratumServer.listen() msg: ', msg);
  console.log(msg);
}).catch(err => {
  console.error(err);
});
