   ...
   */
   ...
   static async handlePeerShareSubmit(shareData ,mongoInterface, poolConfig)
   {
    ...
    let extraNonce = shareData.extraNonce
     ...
     if(!isValid){
      console.log('66666 handlePeerShareSubmit Invalid share 6666', minerEthAddress);
      return { success: false, message: "Invalid RandomX solution" };
     }
     else {
      console.log('888888 handlePeerShareSubmit VALID share 8888888', minerEthAddress);
     }
     

     if (randomx_blob != poolRandomxBlob) {
      console.log('Submitted randomx_blob does not match pools randomx_blob');
