...
/* 
   ...
   */
   ...
   static async handlePeerShareSubmit(shareData ,mongoInterface, poolConfig)
   {
    ...
    let extraNonce = shareData.extraNonce
     ...
     let poolEpochCount = await TokenDataHelper.getEpochCount(mongoInterface);
      ...
      console.log(' -- > newVerifyminerBlob:', newVerifyminerBlob.toString('hex'));
      
      const randomxhashRaw = randomxHelper.convertToRawWithout0x(randomxhash);
      console.log('randomxhashRaw:', randomxhashRaw.toString('hex'));
    ...
    /* replaced by check with actualTarget (actualTargetBN) below
    ...
    */
    ...
    const randomxhashBN = web3utils.toBN(randomxhash);
    console.log('randomxhashBN is: ', randomxhashBN.toString());
    const randomxhashBNString = randomxhashBN.toString('hex');
    console.log('randomxhashBNString is: ', randomxhashBNString);
    
    // Reverse the bytes of randomxhashBN
    const reversedRandomxhash = Buffer.from(randomxhashBNString, 'hex').reverse();
    console.log('reversedRandomx is: ', reversedRandomxhash.toString('hex'));
    const reversedRandomxhashString = reversedRandomxhash.toString('hex');
    ...
    // at this stage claimedtarget contains the default miner minimum shares target
    // now we have calculated the actual target of the share submited by miner and we use it as input for prehandleValidShare
    // eveywhere in softawre the word claimedtarget is still used even if here we have used the term actualTarget but after this stage
    // all references to claimedTarget actually contain actualTarget value
    let actualTargetBN = reversedRandomxhashBN;
    console.log('actualTarget is: ', actualTargetBN.toString());

     // Check if the actual target meets the pool minimum difficulty
     if (actualTargetBN.gt(minShareTarget)) {
      console.log('*-*-*-*-*  ACTUAL TARGET TOO LOW *-*-**-*-*-**-');
      return { success: false, message: "Share acutal Target too low" };
    }

    // Calculate actual difficulty
    const maxTarget = web3utils.toBN(2).pow(web3utils.toBN(256)).sub(web3utils.toBN(1));
    const actualDifficultyBN = maxTarget.div(actualTargetBN);
    console.log('Actual Difficulty:', actualDifficultyBN.toString());
      ...
      const verifiedminerBlob = randomxHelper.convertAdd0x(newVerifyminerBlob.toString('hex')); //adds 0x to fit pool db storage standards
        ...
        return await this.prehandleValidShare( nonce, 
                                            ...
                                            randomx_blob,
                                            randomx_seedhash,
                                            actualTargetBN.toString(),
                                            verifiedminerBlob,
                                            extraNonce,
                                            minerEthAddress,
                                            digest,
                                            actualDifficultyBN.toString(),
...
//   NEW SYSTEM REWARD BASED ON ADDRESSES INSTEAD OF SHARES, REMOVE MINER_SHARES ONLY USES MINER_PENDINGSHARES   //
   ...
   static async  prehandleValidShare( nonce, randomxhash, randomx_blob, randomx_seedhash, claimedtarget, blobWithextraNonce, extraNonce, minerEthAddress,digest,difficulty, shareIsASolution, minerport, mongoInterface, poolConfig )
   {

    console.log('prehandleValidShare function input parameters:');
    console.log('nonce:', nonce);
    console.log('randomxhash:', randomxhash);
    console.log('randomx_blob:', randomx_blob);
    console.log('randomx_seedhash:', randomx_seedhash);
    console.log('claimedtarget:', claimedtarget);
    console.log('blobWithextraNonce:', blobWithextraNonce);
    console.log('extraNonce:', extraNonce);
    console.log('minerEthAddress:', minerEthAddress);
    console.log('digest:', digest);
    console.log('difficulty:', difficulty);
    console.log('shareIsASolution:', shareIsASolution);
    console.log('minerport:', minerport);
      ...
      console.log('existing share is:', existingPreShare);
      console.log('minerEthAddress is:', minerEthAddress);
      ...
      { 
        ...
        // Set limit of maximum one share submission per 5 seconds on port 8080:
       if( (timeToFindShare >= 2 && minerport == 8080) || (timeToFindShare >= 5 && minerport == 8081) || isfirstsolution)
        { 
        await mongoInterface.updateOne('minerData', {_id: minerData._id},  {lastSubmittedSolutionTime: PeerHelper.getTimeNowSeconds()}    )
...
console.log('**-**-***-***-**-*-**-*-*-*-*- > INSERTING SHARE DATA in miner_pendingshares:', shareData);
await mongoInterface.insertOne("miner_pendingshares", shareData ); // saves share data to then use it in handlevalide share
console.log('**-**-***-***-**-*-**-*-*-*-*- > JUST AFTER INSERTING SHARE DATA in miner_pendingshares');
...
await mongoInterface.insertOne('miner_challengediff', newminerchallengediff);
  ...
  await mongoInterface.updateOne('miner_challengediff', {minerEthAddress: minerEthAddress.toString().toLowerCase(), challengeNumber: challengeNumber, minerport: minerport},  {totaldiff: _newminertotaldiff});
        ...
        console.log('**-**-***-***-**-*-**-*-*-*-*- > SHARE ALREADY IN DATABASE not INSERTING in miner_pendingshares');
        return {success: false, message: "This share digest was already received"}
      }
