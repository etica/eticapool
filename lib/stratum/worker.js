        ...
        switch (interaction.method) {
            ...
            case 'authorize':
                ...
                if (!this.subscribe) {
                    ...
                    if (!web3utils.isAddress(_minerEthAddress.toString()) || this.poolSuspended) {
              ...
              case 'submitShare':
                try{
                   ...
                   //let poolSuspended = await PoolStatsHelper.poolMintingIsSuspended(poolConfig,mongoInterface)
                      ...
                      var nonce = interaction.params[0].toString()
                      var minerEthAddress = interaction.params[1].toString().substr(0, 42);
                      var fullWorkerName = interaction.params[1].substring(42).trim();
                      var workerName = fullWorkerName;
                      var chain = ''; // Default empty chain

                      var digest = interaction.params[2].toString()
                      var difficulty = interaction.params[3].toString()
                      var challenge_number = interaction.params[4].toString()
                      var custom_vardiff_used = interaction.params[5].toString();
                      ...
                      console.log("workerName", workerName);
                      console.log("chain", chain); // Log the extracted chain
                      //console.log("poolConfig", this.poolConfig);
                    ...
                    // ------- PoolSuspended -------- //
                      ...
                      var minShareDifficulty = PeerHelper.getPoolMinimumShareDifficultyHard(this.poolConfig);
                      if( difficulty <  minShareDifficulty)
                      {
                        validJSONSubmit = false;
                      }

                      var poolEthAddress_I2 = PeerHelper.getPoolEthAddress(this.poolConfig) ; // cannot re use var name poolEthAddress so named it poolEthAddress_I2 (for Instance 2)
                      var poolChallengeNumber = await TokenDataHelper.getChallengeNumber(this.mongoInterface);
                      var computed_digest_I2 =  web3utils.soliditySha3( poolChallengeNumber , poolEthAddress_I2, nonce );
