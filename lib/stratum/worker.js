...
class Worker extends events.EventEmitter {
    ...
    sendNewJob(increaseJobId=true) {
        ...
        const randomxBlobRaw = randomxHelper.convertToRawWithout0x(this.randomxBlob);
    ...
    async sendFirstJob() {
        ...
        const randomxSeedhashRaw = randomxHelper.convertToRawWithout0x(this.randomxSeedhash);
        console.log('- - - - - - - > compactTarget is: ', compactTarget);
        currentHeight = await TokenDataHelper.getEpochCount(this.mongoInterface);
    ...
    async reloadWorkerJob(){
        ...
        this.challengeNumber = await TokenDataHelper.getChallengeNumber(mongoInterface);
        this.randomxBlob = await TokenDataHelper.getRandomxBlob(mongoInterface);
        this.randomxSeedhash = await TokenDataHelper.getRandomxSeedhash(mongoInterface);
        this.epochCount = await TokenDataHelper.getEpochCount(mongoInterface);
