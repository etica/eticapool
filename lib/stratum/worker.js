...
class Worker extends events.EventEmitter {
    constructor(socket, poolConfig, poolStatsHelper, tokenDataHelper, mongoInterface) {
        ...
        const target1= poolConfig.recentMiningContractData.miningTarget.toString('hex');
        const target2 = web3utils.toBN(poolConfig.recentMiningContractData.miningTarget).toString('hex');
        const target3 = web3utils.toBN(poolConfig.recentMiningContractData.miningTarget).toString(16);
        const hexNumber = BigInt(poolConfig.recentMiningContractData.miningTarget.toString('hex')).toString(16);
        console.log('hexNumber is: ', hexNumber); // Output will be the number in hexadecimal format
        console.log('target1: ', target1);
        console.log('target2: ', target2);
        console.log('target3: ', target3);
        this.target = web3utils.toBN(poolConfig.recentMiningContractData.miningTarget).toString('hex');
        this.jobId = 1;
    ...
    sendInteraction(interaction) {
        ...
        console.log('sending interaction to miner');
        console.log('sending interaction to miner jsonInteraction: ', jsonInteraction);
        ...
        console.log('sending Intercation interactionCall: ', interactionCall)
    ...
    sendNewJob(increaseJobId=true) {
        ...
        console.log('seding notifitication to miner! ');
        console.log('seding notifitication to miner this.challengeNumber: ', this.challengeNumber);
    ...
    async handleUpdate(updatedChallenge) {
        ...
        console.log('Worker is handling updated data:', updatedChallenge);

        try {
        if (updatedChallenge){
            console.log('updatedChallenge PASSED');
            if(this.subscribe){
                console.log('this.subscribe true');
            }
            if(this.authorized){
                console.log('this.authorized true');
            }
            if(updatedChallenge !== '5'){
                console.log('updatedChallenge !== 5 true');
            }
            console.log('updatedChallenge !== 5 && this.authorized && this.subscribe', updatedChallenge !== '5' && this.authorized && this.subscribe);
            if (updatedChallenge !== '5' && this.authorized && this.subscribe) {
                console.log('handleUpdate sendNewJob is reached!!!');
