...
class Worker extends events.EventEmitter {
    ...
    async setMinerSpecificTarget() {
        ...
        if (minerData && minerData.avgHashrate && minerData.avgHashrate > 0) {
            ...
            if (bareAddress) {
              let accountData = minerData;
              // If fullWorkerName differs from bare address, fetch the account-level minerData
              if (this.fullWorkerName.toString().toLowerCase() !== bareAddress) {
                accountData = await PeerHelper.getMinerData(bareAddress, this.mongoInterface);
              }
