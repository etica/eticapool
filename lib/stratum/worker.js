...
class Worker extends events.EventEmitter {
    ...
    async setMinerSpecificTarget() {
        ...
        let minerData = await PeerHelper.getMinerData(this.minerEthAddress.toString().toLowerCase(), this.mongoInterface);
        ...
        if (minerData && minerData.avgHashrate && minerData.avgHashrate > 0) {
            ...
            console.log('poolConfig.minimumTarget is: ', poolConfig.minimumTarget);
            const targetValue = Math.max(minerData.avgHashrate * this.newShareExpectedTime, poolConfig.minimumTarget);
            ...
            this.target = '0x' + web3utils.toBN(targetValueString).toString('hex');
