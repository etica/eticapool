...
class Worker extends events.EventEmitter {
    constructor(socket, poolConfig, poolStatsHelper, tokenDataHelper, mongoInterface) {
        ...
        const target1= valueToConvert.toString('hex');
        const target2 = web3utils.toBN(valueToConvert).toString('hex');
        const target3 = web3utils.toBN(valueToConvert).toString(16);
        const hexNumber = BigInt(valueToConvert).toString(16);
        console.log('hexNumber is: ', hexNumber); // Output will be the number in hexadecimal format
        console.log('target1: ', target1);
        console.log('target2: ', target2);
        console.log('target3: ', target3);
        this.target = web3utils.toBN(valueToConvert).toString('hex');
        this.jobId = 1;
