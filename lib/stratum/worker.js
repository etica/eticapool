...
class Worker extends events.EventEmitter {
    constructor(socket, poolConfig, poolStatsHelper, tokenDataHelper, mongoInterface, redisInterface) {
        super();
        ...
        this.minShareSubmissionInterval = 50; // 50 milliseconds
        ...
        // Add a flag to track socket state
        this.socketClosed = false;

        // Modify socket event listeners
    ...
    // Modify sendInteraction to check socket state
    ...
    async sendFirstJob() {
      try {
        ...
        this.setMinerNameDetails(this.fullWorkerName, this.minerEthAddress, this.workerName);
      } catch (error) {
        console.error(`sendFirstJob error for miner ${this.fullWorkerName || 'unknown'}:`, error);
        this.sendInteraction(new ErrorAnswer(1, errors.UNKNOWN));
      }
    }
    ...
    async _processInteraction(interaction, poolStatsHelper, tokenDataHelper) {
        // Here you can define logic to process different types of interactions
            ...
            try {
                    await this.loginRateLimiter.consume(this.ipAddress, 2);
            } catch (rejRes) {
                //console.log(`Rate limit exceeded for ${this.ipAddress}, method: ${interaction.method}`);
                this.sendInteraction(new ErrorAnswer(interaction.id, errors.RATE_LIMIT_EXCEEDED));
                return;
            }
            ...
            if (interaction.method === 'login'){
                ...
                try{
                   // ------------  Check not too many shares within  minShareSubmissionInterval to protect server resources ------------  //
                    ...
                    console.log("Too many shares from miner: ", this.fullWorkerName, 'difficulty:', this.difficulty);
                    ...
                    validJSONSubmit = false;
                      ...
                      var shareData = {block: ethBlock,
                        ...
                        customVardiff: this.usingCustomDifficulty || false,
                        minerport: 8888,
                      ...
                      if(validJSONSubmit){
                        if (this.redisInterface) {
                          try {
                            await this.redisInterface.addShare(shareData);
                          } catch (redisErr) {
                            console.error('Redis addShare failed, falling back to MongoDB:', redisErr.message);
                            await this.mongoInterface.insertOne('queued_shares_list', shareData);
                          }
                        } else {
                          await this.mongoInterface.insertOne('queued_shares_list', shareData);
                        }
                    ...
                    else if(interaction.method === 'keepalived'){

                        try {

                            // todo:
                            // Not fully implemented yet, will need to implement a automatic disconnection mechanism for inactive miners
                            // and deactivate this mechanism if keepalived is called
                            // Reset the inactivity timer for this miner
                            // this.lastActivityTime = Date.now();
                        
                            // Optionally, send a response back to the miner
                            this.sendInteraction(new PositiveAnswer(interaction.id));
                        
                            //console.log(`Received keepalive from miner: ${this.minerEthAddress}`);
                          } catch (error) {
                        ...
                        console.log("Unknown method name interaction.method is: ", interaction.method);
                        //console.log("Unknown method name interaction is: ", interaction);
    ...
    async setMinerSpecificTarget() {
        ...
        let portMinimumDifficulty;
        if (this.entryport == 3333) {
           portMinimumDifficulty = 400015;
        } else if (this.entryport == 5555) {
           portMinimumDifficulty = 500001;
        } else if (this.entryport == 7777) {
           portMinimumDifficulty = 1000001;
        } else if (this.entryport == 9999) {
            portMinimumDifficulty = 2000001;
        } else {
           portMinimumDifficulty = this.poolConfig.minimumTarget;
        }

        // Check for custom mining difficulty on the bare address (account-level setting)
        let effectiveFloor = portMinimumDifficulty;
        this.usingCustomDifficulty = false;

        const bareAddress = this.minerEthAddress ? this.minerEthAddress.toString().toLowerCase() : null;
        if (bareAddress) {
          let accountData = await PeerHelper.getMinerData(bareAddress, this.mongoInterface);
          if (accountData && accountData.customMiningDifficulty && accountData.customMiningDifficulty > 0) {
            effectiveFloor = Math.max(accountData.customMiningDifficulty, portMinimumDifficulty);
            if (accountData.customMiningDifficulty >= portMinimumDifficulty) {
              this.usingCustomDifficulty = true;
            }
          }
        }
        ...
        if (minerData && minerData.avgHashrate && minerData.avgHashrate > 0) {
            ...
            const minimumTarget = maxTarget.dividedBy(new BigNumber(effectiveFloor));
            ...
            // No hashrate data yet — use the effective floor (custom diff or port minimum)
            const floorTarget = maxTarget.dividedBy(new BigNumber(effectiveFloor));
            this.target = '0x' + floorTarget.integerValue().toString(16).padStart(64, '0');
            this.difficulty = effectiveFloor;
        }
    }
    ...
    async setMinerNameDetails(_fullWorkerName, _addressBlockchain, _workerName) {

                if (!_fullWorkerName || !_addressBlockchain) {
                    console.log('setMinerNameDetails. Missing required parameters');
                    return;
                }

                const _fullWorkerNameFormatted = _fullWorkerName.toString().toLowerCase();

                const minerData = await PeerHelper.getMinerData(_fullWorkerNameFormatted, this.mongoInterface);
                

                if (!minerData) {
                    console.log('setMinerNameDetails. Miner data not found');
                    return;
                }

                if (!web3utils.isAddress(_addressBlockchain.toString())) {
                    console.log('setMinerNameDetails. Invalid blockchain address');
                    return;
                } 

                if(!minerData.addressBlockchain || (_workerName && !minerData.workerName)){

                    minerData.addressBlockchain = _addressBlockchain.toString().toLowerCase();

                if (_workerName && !minerData.workerName) {
                    await this.mongoInterface.updateOne(
                        'minerData',
                        { minerEthAddress: _fullWorkerNameFormatted},
                        { addressBlockchain: minerData.addressBlockchain, workerName: _workerName }
                    );
                }
                else{
                    await this.mongoInterface.updateOne(
    ...
    // Modify _handleError to be more graceful
    _handleError(err) {
        // Don't emit error for common connection-related errors
    ...
    // Modify _handleEnd to ensure cleanup only happens once
    _handleEnd() {
        if (!this.socketClosed) {
            this.socketClosed = true;
            this.stopRetargetTimer();
