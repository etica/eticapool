...
class Worker extends events.EventEmitter {
    constructor(socket, poolConfig, poolStatsHelper, tokenDataHelper, mongoInterface) {
        ...
        console.log(' ---------------3364---------------------------> this.difficulty ', this.difficulty );
        console.log(' --------------3364----------------------------> typeof this.difficulty ', typeof this.difficulty );
        ...
        this.jobId = 1;
        this.newShareExpectedTime = 60 * 10; // miners are expected to send new shares on average every 60 seconds
        this.lastShareSubmissionTime = 0;
        this.minShareSubmissionInterval = 10000; // 10 seconds in milliseconds
              ...
              case 'submit':
                try{
                   // ------------  Check not too many shares within  minShareSubmissionInterval to protect server resources ------------  //
                    ...
                    const currentrandomxBlob = await TokenDataHelper.getRandomxBlob(this.mongoInterface);
                    console.log('minerBlob:', this.randomxBlob.toString('hex'));
                    console.log('networkBlob:', currentrandomxBlob.toString('hex'));
