...
class Worker extends events.EventEmitter {
    constructor(socket, poolConfig, poolStatsHelper, tokenDataHelper, mongoInterface) {
        ...
        this.jobId = 1;
        this.newShareExpectedTime = 30 * 1000; // miners are expected to send new shares on average every 30 seconds
    ...
    async setMinerSpecificTarget() {
        ...
        if (minerData && minerData.avgHashrate && minerData.avgHashrate > 0) {
            ...
            const calculatedTargetValue = new BigNumber(minerData.avgHashrate).multipliedBy(this.newShareExpectedTime);
            console.log('calculatedTargetValue is:', calculatedTargetValue.toString());
            
            // Convert poolConfig.minimumTarget to BigNumber
            const minimumTarget = new BigNumber(this.poolConfig.minimumTarget.toString());
            console.log('poolConfig.minimumTarget as BigNumber is: ', minimumTarget.toString());
            
            // Take the maximum of the two
            const finalTargetValue = BigNumber.max(calculatedTargetValue, minimumTarget);
            console.log('finalTargetValue is:', finalTargetValue.toString());
            
            // Convert finalTargetValue to a string that BN.js can handle
            const integerString = finalTargetValue.toFixed(0); // Convert to integer string without scientific notation
            console.log('finalTargetValue integerString is:', integerString);
