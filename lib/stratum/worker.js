...
class Worker extends events.EventEmitter {
   ...
   /* convertTargetToCompact(target) {
        ...
        let significantBytes = 32;
        for (let i = 0; i < 32; i++) {
            if (target.slice(i*2, i*2+2) !== '00') {
                significantBytes = 32 - i;
                break;
            }
        }
        
        // Take the first 3 significant bytes
        let compact = target.slice(-6);
        
        // Add the number of significant bytes as the first byte
        compact = significantBytes.toString(16).padStart(2, '0') + compact;
        ...
        testCompactTargetConversion() {
            ...
            difficulties.forEach(difficulty => {
                ...
                const compactTarget = convertTargetToCompact(fullTarget.toString(16));
