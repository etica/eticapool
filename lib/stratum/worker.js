...
class Worker extends events.EventEmitter {
    constructor(socket, poolConfig, poolStatsHelper, tokenDataHelper, mongoInterface) {
        ...
        this.jobId = 1;
        this.newShareExpectedTime = 30; // miners are expected to send new shares on average every 30 seconds
    ...
    async sendNewJob(increaseJobId=true) {
        ...
        console.log('- - - - - - - > sending new job this.target before is: ', this.target);
        await this.setMinerSpecificTarget();
        console.log('- - - - - - - > sending new job new this.target is: ', this.target);
              ...
              case 'submit':
                try{
                   // ------------  PoolSuspended   ------------  //
                     ...
                     const minerBlobBuffer = Buffer.from(minerBlob, 'hex');
    ...
    async setMinerSpecificTarget() {
        ...
        if (minerData && minerData.avgHashrate && minerData.avgHashrate > 0) {
            ...
            console.log('minerData.avgHashrate * this.newShareExpectedTime is:', minerData.avgHashrate * this.newShareExpectedTime);
            console.log('poolConfig.minimumTarget is: ', this.poolConfig.minimumTarget);
            const targetValue = Math.max(minerData.avgHashrate * this.newShareExpectedTime, this.poolConfig.minimumTarget);
            console.log('miner TargetValue is:', targetValue);
            const targetValueString = targetValue.toString();
            console.log('miner targetValueString is:', targetValueString);

            // Use BigNumber to handle the large number
            const bigNumberValue = new BigNumber(targetValueString);

            // Convert it to an integer string that BN.js can accept
            const integerString = bigNumberValue.toFixed(0); // Convert to integer string without scientific notation
            console.log('miner integerString is:', integerString);

            this.target = '0x' + web3utils.toBN(integerString).toString('hex');
