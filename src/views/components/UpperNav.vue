    ...
    <t-dropdown  class="w-full" >
      ...
      <div slot-scope="{ hide, blurHandler }" class="origin-top-right absolute right-0 w-56 shadow-lg text-md bg-gray-200 text-gray-900">
        ...
        v-for="row in item.rows"
         :key="row.title"
        class="block w-full  text-black border-2 border-gray-100 px-4 py-2 leading-5 hover:bg-blue-400 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out"
        role="menuitem"
        @blur="blurHandler"
        >
          {{ row.title }}
