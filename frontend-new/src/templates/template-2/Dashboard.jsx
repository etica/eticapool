...
function LiveDot() {
  return (
    <div className="flex items-center gap-2">
      <div className="ecovise-pulse-dot" />
...
function Sparkline() {
  return <div className="ecovise-sparkline w-full h-5 mt-2 rounded" />;
}
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="absolute flex flex-col items-center justify-center" style={{ width: size, height: size }}>
        <span className="text-2xl font-bold text-[#00ff41] ecovise-glow-text">{percent}%</span>
  ...
  return (
    <div className="flex items-end gap-1.5 h-16">
      {heights.map((h, i) => (
        <div
          key={i}
          className="ecovise-bar w-5 rounded-t"
          style={{
            height: `${h}%`,
...
function PortBadge({ port }) {
  return (
    <span className={`ecovise-port-${port} text-xs font-semibold px-2 py-0.5 rounded-full`}>
      {port}
    </span>
  );
}
...
function InfoItem({ label, children, mono }) {
  return (
    <div>
      <div className="ecovise-info-label">{label}</div>
      <div className={mono ? 'ecovise-info-value-mono' : 'ecovise-info-value'}>{children}</div>
    </div>
  );
}
...
function ExpandableHash({ label, value }) {
  ...
  return (
    <div className="mb-3">
      <div className="ecovise-info-label mb-1">{label}</div>
      <div
        className={`ecovise-mono-field ${expanded ? '' : 'ecovise-expandable'} ${expanded ? 'expanded' : ''}`}
...
export default function Dashboard() {
  ...
  return (
    <div className="ecovise-dark min-h-screen text-[#e5e7eb] font-['Inter',sans-serif]">
      {/* ========== TOP BAR / HEADER ========== */}
      <header className="ecovise-section border-b border-emerald-900/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-4">
              <img
                ...
                alt="ETI"
                className="ecovise-logo-img w-10 h-10"
              />
              <div>
                <div className="flex items-center gap-3">
                  <span className="text-xl font-bold text-white ecovise-logo-glow tracking-tight">
                    {poolNameAndUrl.poolName}
                  </span>
                  <span className="ecovise-status-active inline-flex items-center gap-1.5">
                    <span className="ecovise-pulse-dot" style={{ width: 6, height: 6 }} />
            ...
            <div className="flex items-center gap-4">
              <input
                ...
                className="ecovise-search-input px-4 py-2 text-sm w-64 hidden sm:block"
                readOnly
              />
              <LiveDot />
              <Link
                to="/"
                className="ecovise-pill text-sm text-[#9ca3af] border border-emerald-900/30 rounded-full px-4 py-1.5"
              >
                &larr; Templates
              </Link>
            </div>
          ...
          <div className="flex flex-wrap gap-3 mt-4">
            {stratumPorts.map((sp) => (
              <div key={sp.port} className="ecovise-port-card flex items-center gap-3">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        {/* ========== HERO STATS ROW ========== */}
        <section className="ecovise-section grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {heroStats.map((stat) => (
            <div key={stat.label} className="ecovise-card p-5">
              <p className="text-[10px] text-[#9ca3af] font-medium tracking-wider uppercase mb-2">
                {stat.label}
              </p>
              <p className="text-2xl font-bold text-[#00ff41] ecovise-glow-text leading-none">
                {stat.value}
              </p>
              <Sparkline />
            </div>
        ...
        {/* ========== GAUGES + NETWORK ROW ========== */}
        <section className="ecovise-section grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Network Share Gauge */}
          <div className="ecovise-card p-6 flex flex-col items-center justify-center">
          ...
          <div className="ecovise-card p-6 flex flex-col items-center justify-center">
            ...
            <p className="text-4xl font-bold text-[#00ff41] ecovise-glow-text-strong leading-none">
          ...
          {/* Network Stats */}
          <div className="ecovise-card p-6">
            <p className="text-xs text-[#9ca3af] font-medium tracking-wider uppercase mb-5">
              Network Data
            </p>
            <div className="space-y-4">
              <div>
                <div className="ecovise-info-label">Block Height</div>
                <div className="text-xl font-bold text-white">{formatNumber(network.blockHeight)}</div>
              </div>
              <div>
                <div className="ecovise-info-label">Network Difficulty</div>
                <div className="text-xl font-bold text-[#10b981]">{formatNumber(network.difficulty)}</div>
              </div>
              <div>
                <div className="ecovise-info-label">ETI Price</div>
                <div className="text-lg font-bold text-[#00ff41] ecovise-glow-text">
                  ${network.etiPriceUsd}
                </div>
                <div className="text-xs text-[#6b7280] mt-0.5">{network.etiPrice} ETH</div>
              </div>
              <div>
                <div className="ecovise-info-label">Gas Price</div>
        ...
        <section className="ecovise-section">
          <div className="ecovise-card p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
              ...
              <InfoItem label="Mining Difficulty">
                <span className="text-lg font-bold text-[#10b981]">{formatNumber(miningContract.miningDifficulty)}</span>
              </InfoItem>
            </div>
          </div>
        </section>
        ...
        <section className="ecovise-section grid grid-cols-1 lg:grid-cols-2 gap-4">
          {/* Minting Account */}
          <div className="ecovise-card p-6">
          ...
          <div className="ecovise-card p-6">
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-4">
              <InfoItem label="Mining Reward">
                <span className="text-lg font-bold text-[#00ff41] ecovise-glow-text">
        ...
        {/* ========== MINER LEADERBOARD ========== */}
        <section className="ecovise-section">
          <div className="ecovise-card p-6 overflow-hidden">
            <SectionHeader title="Top Miners" />
            <div className="overflow-x-auto ecovise-scroll">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {miners.slice(0, 10).map((miner, idx) => (
                    <tr key={miner.minerEthAddress} className="ecovise-table-row">
                      ...
                      <td className="py-3 px-3 text-right text-[#00ff41] font-medium ecovise-glow-text">
        ...
        <section className="ecovise-section grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="ecovise-card p-6">
            ...
            <div className="space-y-2 ecovise-scroll max-h-96 overflow-y-auto pr-1">
              {recentBlocks.map((block) => (
                <div
                  key={block.epoch}
                  className="ecovise-accent-left rounded-r-lg px-4 py-3 bg-[#0a0f0a]/50"
                >
            ...
            <div className="space-y-2 ecovise-scroll max-h-96 overflow-y-auto pr-1">
              {payouts.map((payout, idx) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span className="text-sm font-mono text-emerald-400">{payout.address}</span>
                      <span className="text-sm font-semibold text-[#00ff41] ecovise-glow-text">
                        {payout.amount} ETI
                      </span>
                    </div>
        ...
        {/* ========== FOOTER ========== */}
        <footer className="ecovise-section pt-2">
          <div className="ecovise-footer-border mb-6" />
