...
function LiveDot() {
  return (
    <div className="flex items-center gap-2">
      <div className="spectrum-pulse-dot" />
...
function Sparkline() {
  return <div className="spectrum-sparkline w-full h-5 mt-2 rounded" />;
}
    ...
    <div className="flex flex-col items-center">
      <svg width={size} height={size} className="transform -rotate-90">
        ...
        <circle
          ...
          r={radius}
          fill="none"
          stroke="url(#spectrumGaugeGradient)"
          strokeWidth={strokeWidth}
          strokeLinecap="round"
          strokeDasharray={circumference}
          strokeDashoffset={offset}
          className="spectrum-ring-circle"
          style={{ '--target-offset': offset }}
        />
        <defs>
          <linearGradient id="spectrumGaugeGradient" x1="0%" y1="0%" x2="100%" y2="0%">
      ...
      <div className="absolute flex flex-col items-center justify-center" style={{ width: size, height: size }}>
        <span className="text-2xl font-bold text-[#00ff41] spectrum-glow-text">{percent}%</span>
  ...
  return (
    <div className="flex items-end gap-1.5 h-16">
      {heights.map((h, i) => (
        <div
          key={i}
          className="spectrum-bar w-5 rounded-t"
          style={{
            height: `${h}%`,
...
function SectionHeader({ title, colorClass = 'spectrum-header-green' }) {
  return (
...
function PortBadge({ port }) {
  return (
    <span className={`spectrum-port-${port} text-xs font-semibold px-2 py-0.5 rounded-full`}>
      {port}
    </span>
  );
}
...
function InfoItem({ label, children, mono }) {
  return (
    <div>
      <div className="spectrum-info-label">{label}</div>
      <div className={mono ? 'spectrum-info-value-mono' : 'spectrum-info-value'}>{children}</div>
    </div>
  );
}
...
function ExpandableHash({ label, value }) {
  ...
  return (
    <div className="mb-3">
      <div className="spectrum-info-label mb-1">{label}</div>
      <div
        className={`spectrum-mono-field ${expanded ? '' : 'spectrum-expandable'} ${expanded ? 'expanded' : ''}`}
...
export default function Dashboard() {
  ...
  return (
    <div className="spectrum-dark min-h-screen text-[#e5e7eb] font-['Inter',sans-serif]">
      {/* ========== TOP BAR / HEADER (GREEN) ========== */}
      <header className="spectrum-section border-b border-emerald-900/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-4">
              <img
                ...
                alt="ETI"
                className="spectrum-logo-img w-10 h-10"
              />
              <div>
                <div className="flex items-center gap-3">
                  <span className="text-xl font-bold text-white spectrum-logo-glow tracking-tight">
                    {poolNameAndUrl.poolName}
                  </span>
                  <span className="spectrum-status-active inline-flex items-center gap-1.5">
                    <span className="spectrum-pulse-dot" style={{ width: 6, height: 6 }} />
            ...
            <div className="flex items-center gap-4">
              <input
                type="text"
                placeholder="0x..."
                className="spectrum-search-input px-4 py-2 text-sm w-64 hidden sm:block"
                readOnly
              />
              <LiveDot />
              <Link
                to="/"
                className="spectrum-pill text-sm text-[#9ca3af] border border-emerald-900/30 rounded-full px-4 py-1.5"
              >
                &larr; Templates
              </Link>
            </div>
          ...
          <div className="flex flex-wrap gap-3 mt-4">
            {stratumPorts.map((sp) => (
              <div key={sp.port} className="spectrum-port-card flex items-center gap-3">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        <section className="spectrum-section grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {heroStats.map((stat) => (
            <div key={stat.label} className="spectrum-card p-5">
              <p className="text-[10px] text-[#9ca3af] font-medium tracking-wider uppercase mb-2">
                {stat.label}
              </p>
              <p className="text-2xl font-bold text-[#00ff41] spectrum-glow-text leading-none">
                {stat.value}
              </p>
              <Sparkline />
            </div>
        ...
        {/* ========== GAUGES + NETWORK ROW (GREEN gauges, BLUE network) ========== */}
        <section className="spectrum-section grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Network Share Gauge (GREEN) */}
          <div className="spectrum-card p-6 flex flex-col items-center justify-center">
          ...
          <div className="spectrum-card p-6 flex flex-col items-center justify-center">
            ...
            <p className="text-4xl font-bold text-[#00ff41] spectrum-glow-text-strong leading-none">
          ...
          {/* Network Stats (BLUE) */}
          <div className="spectrum-card-blue p-6">
            <SectionHeader title="Network Data" colorClass="spectrum-header-blue" />
            <div className="space-y-4">
              <div>
                <div className="spectrum-info-label">Block Height</div>
                <div className="text-xl font-bold text-[#3b82f6]">{formatNumber(network.blockHeight)}</div>
              </div>
              <div>
                <div className="spectrum-info-label">Network Difficulty</div>
                <div className="text-xl font-bold text-[#60a5fa]">{formatNumber(network.difficulty)}</div>
              </div>
              <div>
                <div className="spectrum-info-label">ETI Price</div>
              ...
              <div>
                <div className="spectrum-info-label">Gas Price</div>
                <div className="text-sm text-[#93c5fd] font-medium">{network.gasPrice} GWei</div>
              </div>
              <div>
                <div className="spectrum-info-label">Epoch</div>
                <div className="text-sm text-[#93c5fd] font-medium">{formatNumber(network.epoch)}</div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="spectrum-section">
          <div className="spectrum-card-purple p-6">
            <SectionHeader title="Mining Contract Data" colorClass="spectrum-header-purple" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
              <InfoItem label="Epoch Count">
                <span className="text-lg font-bold text-[#c084fc]">{formatNumber(miningContract.epochCount)}</span>
              </InfoItem>
              <InfoItem label="Mining Difficulty">
                <span className="text-lg font-bold text-[#a855f7]">{formatNumber(miningContract.miningDifficulty)}</span>
              </InfoItem>
            </div>
          </div>
        </section>

        {/* ========== ACCOUNT / FINANCIAL INFO (TEAL minting, CYAN payments) ========== */}
        <section className="spectrum-section grid grid-cols-1 lg:grid-cols-2 gap-4">
          {/* Minting Account (TEAL) */}
          <div className="spectrum-card-teal p-6">
            <SectionHeader title="Minting Account" colorClass="spectrum-header-teal" />
            <div className="space-y-4">
          ...
          {/* Payments Account (CYAN) */}
          <div className="spectrum-card-cyan p-6">
            <SectionHeader title="Payments Account" colorClass="spectrum-header-cyan" />
            <div className="space-y-4">
        ...
        {/* ========== CONTRACTS + FEES (TEAL contracts, AMBER fees) ========== */}
        <section className="spectrum-section grid grid-cols-1 lg:grid-cols-2 gap-4">
          {/* Contract Addresses (TEAL) */}
          <div className="spectrum-card-teal p-6">
            <SectionHeader title="Contracts" colorClass="spectrum-header-teal" />
            <div className="space-y-4">
          ...
          {/* Pool Fees / Metrics (AMBER) */}
          <div className="spectrum-card-amber p-6">
            <SectionHeader title="Pool Fees / Status" colorClass="spectrum-header-amber" />
        ...
        {/* ========== MINER LEADERBOARD (WHITE text, green hashrate only) ========== */}
        <section className="spectrum-section">
          <div className="spectrum-card p-6 overflow-hidden" style={{ borderColor: 'rgba(255, 255, 255, 0.1)' }}>
            <SectionHeader title="Top Miners" colorClass="spectrum-header-white" />
            <div className="overflow-x-auto spectrum-scroll">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {miners.slice(0, 10).map((miner, idx) => (
                    <tr key={miner.minerEthAddress} className="spectrum-table-row">
                      ...
                      <td className="py-3 px-3 text-right text-[#00ff41] font-medium spectrum-glow-text">
                      ...
                      <td className="py-3 px-3 text-right text-[#d1d5db]">
        ...
        {/* ========== RECENT BLOCKS (GREEN accent) + RECENT PAYOUTS (BLUE accent) ========== */}
        <section className="spectrum-section grid grid-cols-1 lg:grid-cols-2 gap-4">
          {/* Recent Blocks (GREEN left-accent) */}
          <div className="spectrum-card p-6">
            <SectionHeader title="Recent Blocks" colorClass="spectrum-header-green" />
            <div className="space-y-2 spectrum-scroll max-h-96 overflow-y-auto pr-1">
              {recentBlocks.map((block) => (
                <div
                  key={block.epoch}
                  className="spectrum-accent-green rounded-r-lg px-4 py-3 bg-[#0a0f0a]/50"
                >
          ...
          {/* Recent Payouts (BLUE left-accent) */}
          <div className="spectrum-card-blue p-6">
            <SectionHeader title="Recent Payouts" colorClass="spectrum-header-blue" />
            <div className="space-y-2 spectrum-scroll max-h-96 overflow-y-auto pr-1">
              {payouts.map((payout, idx) => (
                <div
                  key={idx}
                  className="spectrum-accent-blue rounded-r-lg px-4 py-3 bg-[#0a0c12]/50"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span className="text-sm font-mono text-[#93c5fd]">{payout.address}</span>
        ...
        {/* ========== FOOTER ========== */}
        <footer className="spectrum-section pt-2">
          <div className="spectrum-footer-border mb-6" />
          <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-2 text-xs text-[#9ca3af]">
            <span>
              Pool Fee: <span className="text-[#e5e7eb] font-medium">{pool.poolFee}%</span>
            </span>
            <span>
              Min Payout: <span className="text-[#e5e7eb] font-medium">{pool.minimumPayout} ETI</span>
            </span>
            <span>
              ETI Price: <span className="text-[#00ff41] font-medium">${network.etiPriceUsd}</span>
            </span>
            <span>
